...
if ($row = mysqli_fetch_assoc($result)) {
  ...
  if (file_exists($profileImagePath)) {
      echo '<img id="profilePicture" src="' . $profileImagePath . '" class="w-32 h-32 bg-gray-300 rounded-full mb-2" alt="Profile Picture">';
  } else {
      echo '<span>Profile picture not found</span>';
  }
  echo '</div>';
  // Display other user details
  echo '<h1 class="text-xl font-bold">' . $row["fname"] . ', ' . $row["Mname"] . ' ' . $row["lname"] . '</h1>';
  // Rest of the user details display code...
}
  echo '<h1 class="text-xl font-bold">' . $row["fname"] . ', ' . $row["Mname"] . ' ' . $row["lname"] . '</h1>';
