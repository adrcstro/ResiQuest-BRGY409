...
$sql = "SELECT 
            ...
            SUM(CASE WHEN ServiceType = 'Certificate of Residency' THEN 1 ELSE 0 END) as residency_count,
            SUM(CASE WHEN ServiceType = 'Certificate of Indigency' THEN 1 ELSE 0 END) as indigency_count,
            SUM(CASE WHEN ServiceType = 'Business Clearance' THEN 1 ELSE 0 END) as business_clearance_count,
            SUM(CASE WHEN ServiceType = 'Barangay Identification' THEN 1 ELSE 0 END) as id_count
...
$residency_count = 0;
$indigency_count = 0;
$business_clearance_count = 0;
$id_count = 0;

if ($row = $result->fetch_assoc()) {
    $request_count = $row['request_count'];
    $residency_count = $row['residency_count'];
    $indigency_count = $row['indigency_count'];
    $business_clearance_count = $row['business_clearance_count'];
...
<table border="1" cellpadding="5">
    <tr>
        ...
        <th>Certificate of Residency</th>
        <th>Certificate of Indigency</th>
        <th>Business Clearance</th>
        <th>Barangay Identification</th>
    </tr>
    <tr>
        <td>' . $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT) . '</td>
        <td>' . $request_count . '</td>
        <td>' . $residency_count . '</td>
        <td>' . $indigency_count . '</td>
        <td>' . $business_clearance_count . '</td>
        <td>' . $id_count . '</td>
    </tr>
</table>';
