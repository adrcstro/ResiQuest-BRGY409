...
if ($row = mysqli_fetch_assoc($result)) {
  ...
  echo '<div class="flex flex-col md:flex-row items-center">';
  echo '<label for="resident-id" class="font-semibold mr-2">Residents ID:</label>';
  echo '<h1 id="resident-id" class="text-lg ">' . $row["ID"] . '</h1>';
  echo '<span class="mx-4 md:hidden">|</span>'; // Hide separator on mobile
  echo '<label for="status" class="font-semibold mr-2 md:mr-4">Status:</label>'; // Add margin for tablet and above
