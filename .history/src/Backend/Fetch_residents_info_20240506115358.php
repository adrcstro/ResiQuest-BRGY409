...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    <img id="profilePicture" src="uploads/' . $row['Profile'] . '" class="w-32 h-32 bg-gray-300 rounded-full mb-4" alt="Profile Picture">
  </div>';
  echo '<h1 class="text-xl font-bold">' . $row["fname"] . ', ' . $row["Mname"] . ' ' . $row["lname"] . '</h1>';
  echo '<h1 class="text-lg font-semibold">' . $row["ID"] . '</h1>';
