...
if(!empty($_SESSION["ID"])) {
    ...
    // Prepare and execute the SQL query to fetch the total number of requests for pagination
    $count_query = "SELECT COUNT(*) AS total FROM requestdocument WHERE UserID = $ID";
    ...
    // Prepare and execute the SQL query to fetch requests for the current page
    $query = "SELECT * FROM requestdocument WHERE UserID = $ID LIMIT $records_per_page OFFSET $offset";
...
echo '<nav aria-label="Page navigation example" class="flex justify-center mt-4">';
echo '<ul class="inline-flex -space-x-px text-sm">';

// Previous button
if ($current_page > 1) {
    echo '<li><a href="?page=' . ($current_page - 1) . '" class="pagination-btn flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</a></li>';
}

// Page numbers
for ($i = 1; $i <= $total_pages; $i++) {
    $activeClass = $i == $current_page ? 'text-blue-600 bg-blue-50 hover:bg-blue-100 hover:text-blue-700' : 'text-gray-500 bg-white hover:bg-gray-100 hover:text-gray-700';
...
if ($current_page < $total_pages) {
    echo '<li><a href="?page=' . ($current_page + 1) . '" class="pagination-btn flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</a></li>';
}
