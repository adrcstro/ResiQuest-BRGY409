...
if(!empty($_SESSION["ID"])) {
    ...
    // Total records query
    $total_query = "SELECT COUNT(*) as total FROM requestdocument WHERE UserID = $ID";
    ...
    $query = "SELECT * FROM requestdocument WHERE UserID = $ID LIMIT $limit OFFSET $offset";
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            ...
            if (!in_array($row["Status"], ["Completed", "Released", "Rejected"])) {
                echo '<tr>';
                ...
                <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
                   ...
                   <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 9.75h10.5m-7.5 3.75h4.5m-9-6h13.5M19.5 6.75l-.637 12.112a2.25 2.25 0 0 1-2.244 2.138H7.38a2.25 2.25 0 0 1-2.244-2.138L4.5 6.75m4.875 0V5.25a1.875 1.875 0 0 1 1.875-1.875h2.25A1.875 1.875 0 0 1 15.375 5.25v1.5" />
                 </svg>
                   Delete Request</a></li>
                   </ul>
                   </div>
        ...
        // Output empty rows if fewer than 5 entries
        $num_rows = mysqli_num_rows($result);
        for ($i = $num_rows; $i < $limit; $i++) {
            echo '<tr>';
            echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">&nbsp;</td>';
            echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">&nbsp;</td>';
            echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">&nbsp;</td>';
            echo '<td class="p-3 border-2 border-inherit text-center text-sm font-semibold text-gray-900">&nbsp;</td>';
            echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">&nbsp;</td>';
            echo '</tr>';
        }
        ...
        echo '<div>';
        if($page > 1) {
            echo '<a href="?page='.($page - 1).'" class="px-3 py-1 rounded bg-gray-300 hover:bg-gray-400">Previous</a>';
        }
        echo '</div>';
        echo '<div>';
        if($page < $total_pages) {
            echo '<a href="?page='.($page + 1).'" class="px-3 py-1 rounded bg-gray-300 hover:bg-gray-400">Next</a>';
        }
        echo '</div>';
        echo '</div>';
    } else {
        // If no entries found, output a message
