...
class PDF extends TCPDF {
    ...
    public function __construct($organizationName, $organizationAddress, $brgyname, $Zonenumname, $fulladdress , $officeofbrgy, $serviceType, $nameofbrgycharman, $psotiontitle , $attestedby, $line, $kagawad, $notvalid) {
        parent::__construct();
    ...
    public function Header() {
        ...
        $this->SetFont('times', 'B', 12); // Set font to bold and size 14
        $this->Cell(0, 10, $this->attestedby, 0, 9, 'R');
        $this->Ln(-5);
        $this->SetFont('times', 'B', 10); // Set font to bold and size 14
        $this->SetX(10); // Set X coordinate to 30 (adjust as needed)
        $this->Cell(0, 10, $this->line, 0, 10, 'C');
        $this->Ln(120); // Adjust the line spacing as needed
        $this->SetFont('times', 'B', 12); // Set font to bold and size 14
        $this->Cell(0, 10, $this->kagawad, 0, 9, 'R');
        $this->Ln(-5);
        $this->SetFont('times', 'B', 10); // Set font to bold and size 14
        $this->SetX(10); // Set X coordinate to 30 (adjust as needed)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf = new PDF($organizationName, $organizationAddress ,$brgyname, $Zonenumname, $fulladdress, $officeofbrgy, $serviceType, $nameofbrgycharman, $psotiontitle,$attestedby,$line,$kagawad,$notvalid);
