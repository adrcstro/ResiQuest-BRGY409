...
FROM requestdocument INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID AND Status='Completed' AND (PickUpDate IS NULL OR PickUpDate = '')";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td class="p-1 border flex items-center justify-center border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">
            <div class="dropdown dropdown-end">
                ...
                <li class="Schedule-resident-pickup"
                data-request-id="' . htmlspecialchars($row["RequestID"]) . '"
                data-request-servietype="' . htmlspecialchars($row["ServiceType"]) . '"
                data-request-Fname="' . htmlspecialchars($row["FirstName"]) . '"
                data-request-Mname="' . htmlspecialchars($row["Middlename"]) . '"
                data-request-Lname="' . htmlspecialchars($row["Lastname"]) . '"
                data-adminid-logs="' . htmlspecialchars($row["AdminID"]) . '"
                data-adminusername-logs="' . htmlspecialchars($row["Lname"]) . ',' . htmlspecialchars($row["Fname"]) . ' ' . htmlspecialchars($row["MName"]) . '"
                data-adminposition-logs="' . htmlspecialchars($row["BrgyPosition"]) . '">
...
<input value="Set the Schedule for Request ${ID}"  type="text" name="Adminactiondone" id="Adminactiondone" autocomplete="street-address" 
    class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 mb-4">
</div>
<div class="hidden">
