    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
          ...
          View Details</a></li>
           ...
           data-Residents-voterstat="' . $row["VoterStatus"] . '"   data-Residents-email="' . $row["email"] . '"   data-Residents-occupation="' . $row["Occupation"] . '"   data-Residents-contact="' . $row["contactNo"] . '"
           data-Residents-address="' . $row["address"] . '"
           ><a>
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 3.487a2.069 2.069 0 0 1 2.926 2.925l-9.9 9.9a4.5 4.5 0 0 1-1.697 1.065l-3.24.97a.5.5 0 0 1-.617-.617l.97-3.24a4.5 4.5 0 0 1 1.065-1.697l9.9-9.9Z" />
           <path stroke-linecap="round" stroke-linejoin="round" d="m18.5 5.5 2 2m-3.378 5.134L11 8.5m-5.5 8.5h6" />
         </svg>
           Edit</a></li>
           <li class="delete-residents-btn"  data-Residents-id="' . $row["ID"] . '"><a>
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M6 7.5h12M9.75 7.5V6a2.25 2.25 0 1 1 4.5 0v1.5m-8.25 0V18A2.25 2.25 0 0 0 8.25 20.25h7.5A2.25 2.25 0 0 0 18 18V7.5H6Z" />
         </svg>
           Delete</a></li>
         </ul>
     </div>
...
echo '<div class="btn-group flex justify-center mt-2">';
for ($i = 1; $i <= $total_pages; $i++) {
    $activeClass = ($i == $page) ? 'btn-active' : '';
    echo '<button class="btn pagination-btn ' . $activeClass . '" data-page="' . $i . '">' . $i . '</button>';
}
echo '</div>';
