...
WHERE admintbl.AdminID = $ID AND Gender='Female'";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
           <li class="open-resident-details-modal-btnFemale" data-Residents-id="' . $row["ID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
           <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
         </svg>
          View Details</a></li>

           <li class="edit-residents-btnFemale"  data-Residents-id="' . $row["ID"] . '"  data-Residents-profile="' . $row["Profile"] . '" data-Residents-fname="' . $row["fname"] . '"  data-Residents-mname="' . $row["Mname"] . '"  data-Residents-Lname="' . $row["lname"] . '" data-Residents-nname="' . $row["Nickname"] . '" data-Residents-placeofbirth="' . $row["PlaceofBirth"] . '"  data-Residents-dateofbirth="' . $row["Dateofbirth"] . '"
          ...
          Edit Residents</a></li>
          <li class="delete-residents-btnFemale" data-Residents-id="' . $row["ID"] . '" data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '" ><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
      ...
      <li class="delete-verify-btnFemale" data-Residents-id="' . $row["ID"] . '"  data-verifyResidents-profile="' . $row["Profile"] . '" data-Residents-fname="' . $row["fname"] . '"  data-Residents-mname="' . $row["Mname"] . '"  data-Residents-Lname="' . $row["lname"] . '"  data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    if ($page > 1) {
        echo '<li><a href="AdminDashboard.php?page=' . ($page - 1) . '" class="py-2 px-3 ml-0 leading-tight text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">Previous</a></li>';
    }
    for ($i = 1; $i <= $total_pages; $i++) {
        if ($i == $page) {
            echo '<li><a href="AdminDashboard.php?page=' . $i . '" class="py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700">' . $i . '</a></li>';
        } else {
            echo '<li><a href="AdminDashboard.php?page=' . $i . '" class="py-2 px-3 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700">' . $i . '</a></li>';
        }
    }
    if ($page < $total_pages) {
        echo '<li><a href="AdminDashboard.php?page=' . ($page + 1) . '" class="py-2 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700">Next</a></li>';
    }
    echo '</ul>';
    echo '</nav>';
    echo '</div>';
//modal for view residents info
    ...
    echo '<dialog id="ViewResidentsdetailsFemale" class="modal p-3">
    <div class="modal-box w-100 max-w-4xl rounded-none">
      ...
      <div id="modal-content-resident-infoFemale" class="mt-8 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row"></div>
      <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
      <button id="close-modal-btnFemale" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
      ✕
    </button>
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", function () {
  const modal = document.getElementById("ViewResidentsdetailsFemale");
  const modalContent = document.getElementById("modal-content-resident-infoFemale");
  const openModalButtons = document.querySelectorAll(".open-resident-details-modal-btnFemale");
  const closeModalButton = document.getElementById("close-modal-btnFemale");
...
echo '<dialog id="EditResidentsDetailsFemale" class="modal p-3 ">
        <div class="modal-box w-100 max-w-4xl rounded-lg">
            ...
            <form id="admineditresidentinfoFemale" action="../Backend/Update.php" method="post" enctype="multipart/form-data">
          <div id="modal-content-editresident-infoFemale"></div>
          </form>

          <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
          <button id="resident-edit-close-modal-btnFemale" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
          ✕
        </button>
        <div class="px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
        
        <button" id="admineditresidentbuttonFemale" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
...
echo '<script>
document.addEventListener("DOMContentLoaded", function () {
  const modal = document.getElementById("EditResidentsDetailsFemale");
  const modalContent = document.getElementById("modal-content-editresident-infoFemale");
  const openModalButtons = document.querySelectorAll(".edit-residents-btnFemale");
  const closeModalButton = document.getElementById("resident-edit-close-modal-btnFemale");
...
echo '<script>
    $(function() {
        $("#admineditresidentbuttonFemale").click(function(e) {
            e.preventDefault();
            var valid = $("#admineditresidentinfoFemale")[0].checkValidity();
            if (valid) {
                var formData = new FormData($("#admineditresidentinfoFemale")[0]);
                $.ajax({
                    type: "POST",
    ...
    document.getElementById("admineditresidentbuttonFemale").addEventListener("click", function() {
        var dialog = document.getElementById("EditResidentsDetailsFemale");
        dialog.close();
    });
</script>';
...
echo '<script>
document.addEventListener("DOMContentLoaded", function () {
    const deleteRequestButtons = document.querySelectorAll(".delete-residents-btnFemale");
...
echo '<dialog id="VerifyresidentsFemale" class="modal p-3 ">
        <div class="modal-box w-70 max-w-3xl rounded-lg">
            ...
            <form id="VerifyresidentinfoFemale" action="../Backend/Update.php" method="post" enctype="multipart/form-data">

          <div id="modal-content-Verifyresident-infoFemale"></div>
          </form>
        ...
        <button" id="VerifyresidentbuttonFemale" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
          </svg>
      ...
      echo '<script>
      document.addEventListener("DOMContentLoaded", function () {
        const modal = document.getElementById("VerifyresidentsFemale");
        const modalContent = document.getElementById("modal-content-Verifyresident-infoFemale");
        const openModalButtons = document.querySelectorAll(".delete-verify-btnFemale");
        const closeModalButton = document.getElementById("Verifyresident-edit-close-modal-btnFemale");
...
echo '<script>
    $(function() {
        $("#VerifyresidentbuttonFemale").click(function(e) {
            e.preventDefault();

            var valid = $("#VerifyresidentinfoFemale")[0].checkValidity();

            if (valid) {
                var formData = new FormData($("#VerifyresidentinfoFemale")[0]);
    ...
    document.getElementById("VerifyresidentbuttonFemale").addEventListener("click", function() {
        var dialog = document.getElementById("VerifyresidentsFemale");
        dialog.close();
    });
</script>';
