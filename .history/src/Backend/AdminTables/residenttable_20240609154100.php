...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043 3.746 3.746 0 0 1-1.043-3.296A3.746 3.746 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.746 3.746 0 0 1 1.043-3.296 3.746 3.746 0 0 1 3.296-1.043A3.746 3.746 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 0 1 3.296 1.043 3.746 3.746 0 0 1 1.043 3.296A3.745 3.745 0 0 1 21 12Z" />
    ...
    // Pagination
    $total_pages_query = "SELECT COUNT(*) as count FROM users INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID";
    $result = mysqli_query($conn, $total_pages_query);
    $total_rows = mysqli_fetch_array($result)['count'];
    $total_pages = ceil($total_rows / 5); // 5 items per page

    echo '<div class="flex justify-center mt-4" id="pagination">';
    echo '<ul class="flex list-none rounded-md">';
    for ($i = 1; $i <= $total_pages; $i++) {
        if ($i == $page) {
            echo '<li class="px-3 py-1 bg-gray-200 text-gray-800 mr-2 rounded-md">' . $i . '</li>';
        } else {
            echo '<li class="px-3 py-1 bg-gray-100 text-gray-800 hover:bg-gray-200 mr-2 rounded-md"><a href="#" onclick="loadData(' . $i . ')">' . $i . '</a></li>';
        }
    }
    echo '</ul></div>';
