...
WHERE admintbl.AdminID = $ID AND Gender='Male'";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
           <li class="open-resident-details-modal-btnMale" data-Residents-id="' . $row["ID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
           <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
         </svg>
          View Details</a></li>

           <li class="edit-residents-btnMale"  data-Residents-id="' . $row["ID"] . '"  data-Residents-profile="' . $row["Profile"] . '" data-Residents-fname="' . $row["fname"] . '"  data-Residents-mname="' . $row["Mname"] . '"  data-Residents-Lname="' . $row["lname"] . '" data-Residents-nname="' . $row["Nickname"] . '" data-Residents-placeofbirth="' . $row["PlaceofBirth"] . '"  data-Residents-dateofbirth="' . $row["Dateofbirth"] . '"
          ...
          Edit Residents</a></li>
          <li class="delete-residents-btnMale" data-Residents-id="' . $row["ID"] . '" data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '" ><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
      ...
      <li class="delete-verify-btnMale" data-Residents-id="' . $row["ID"] . '"  data-verifyResidents-profile="' . $row["Profile"] . '" data-Residents-fname="' . $row["fname"] . '"  data-Residents-mname="' . $row["Mname"] . '"  data-Residents-Lname="' . $row["lname"] . '"  data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
//modal for view residents info
    ...
    echo '<dialog id="ViewResidentsdetailsMale" class="modal p-3">
    <div class="modal-box w-100 max-w-4xl rounded-none">
      ...
      <div id="modal-content-resident-infoMale" class="mt-8 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row"></div>
      <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
      <button id="close-modal-btnMale" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
      ✕
    </button>
      </div>
    </div>
...
document.addEventListener("DOMContentLoaded", function () {
  const modal = document.getElementById("ViewResidentsdetailsMale");
  const modalContent = document.getElementById("modal-content-resident-infoMale");
  const openModalButtons = document.querySelectorAll(".open-resident-details-modal-btnMale");
  const closeModalButton = document.getElementById("close-modal-btnMale");
...
echo '<dialog id="EditResidentsDetailsMale" class="modal p-3 ">
        <div class="modal-box w-100 max-w-4xl rounded-lg">
            ...
            <form id="admineditresidentinfoMale" action="../Backend/Update.php" method="post" enctype="multipart/form-data">
          <div id="modal-content-editresident-infoMale"></div>
          </form>

          <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
          <button id="resident-edit-close-modal-btnMale" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
          ✕
        </button>
        <div class="px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
        
        <button" id="admineditresidentbuttonMale" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
...
echo '<script>
document.addEventListener("DOMContentLoaded", function () {
  const modal = document.getElementById("EditResidentsDetailsMale");
  const modalContent = document.getElementById("modal-content-editresident-infoMale");
  const openModalButtons = document.querySelectorAll(".edit-residents-btnMale");
  const closeModalButton = document.getElementById("resident-edit-close-modal-btnMale");
...
echo '<script>
    $(function() {
        $("#admineditresidentbuttonMale").click(function(e) {
            e.preventDefault();
            var valid = $("#admineditresidentinfoMale")[0].checkValidity();
            if (valid) {
                var formData = new FormData($("#admineditresidentinfoMale")[0]);
                $.ajax({
                    type: "POST",
    ...
    document.getElementById("admineditresidentbuttonMale").addEventListener("click", function() {
        var dialog = document.getElementById("EditResidentsDetailsMale");
        dialog.close();
    });
</script>';
...
echo '<script>
document.addEventListener("DOMContentLoaded", function () {
    const deleteRequestButtons = document.querySelectorAll(".delete-residents-btnMale");
...
echo '<dialog id="VerifyresidentsMale" class="modal p-3 ">
        <div class="modal-box w-70 max-w-3xl rounded-lg">
            ...
            <form id="VerifyresidentinfoMale" action="../Backend/Update.php" method="post" enctype="multipart/form-data">

          <div id="modal-content-Verifyresident-infoMale"></div>
          </form>

          <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
          <button id="Verifyresident-edit-close-modal-btnMale" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
          ✕
        </button>
        <div class="px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
        
        <button" id="VerifyresidentbuttonMale" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
          </svg>
      ...
      echo '<script>
      document.addEventListener("DOMContentLoaded", function () {
        const modal = document.getElementById("VerifyresidentsMale");
        const modalContent = document.getElementById("modal-content-Verifyresident-infoMale");
        const openModalButtons = document.querySelectorAll(".delete-verify-btnMale");
        const closeModalButton = document.getElementById("Verifyresident-edit-close-modal-btnMale");
...
echo '<script>
    $(function() {
        $("#VerifyresidentbuttonMale").click(function(e) {
            e.preventDefault();

            var valid = $("#VerifyresidentinfoMale")[0].checkValidity();

            if (valid) {
                var formData = new FormData($("#VerifyresidentinfoMale")[0]);
    ...
    document.getElementById("VerifyresidentbuttonMale").addEventListener("click", function() {
        var dialog = document.getElementById("VerifyresidentsMale");
        dialog.close();
    });
</script>';
