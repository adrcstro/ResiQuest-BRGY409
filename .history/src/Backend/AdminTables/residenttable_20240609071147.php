...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
           ...
           <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1
.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
    ...
    $queryCount = "SELECT COUNT(*) as total FROM users INNER JOIN admintbl ON admintbl.AdminID = users.AdminID WHERE admintbl.AdminID = $ID";
    $resultCount = mysqli_query($conn, $queryCount);
    $totalRecords = mysqli_fetch_assoc($resultCount)['total'];
    $hasPrev = $offset > 0;
    $hasNext = $totalRecords - $offset > $limit;
    echo '<div class="flex justify-between mt-4">';
    if ($hasPrev) {
        echo '<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onclick="loadData(' . $prevOffset . ')">Previous</button>';
    }
    if ($hasNext) {
        echo '<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onclick="loadData(' . $nextOffset . ')">Next</button>';
    }
    echo '</div>';
