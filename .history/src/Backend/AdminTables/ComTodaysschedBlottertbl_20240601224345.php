  ...
  echo '<td class="p-3 border border-inherit text-center   whitespace-nowrap text-sm font-semibold text-gray-900">
    <div class="dropdown dropdown-end">
        ...
        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
            ...
            <li class="markasswtteledToday" data-setscheduleblootter-id="' . $row["ComplainID"] . '" data-request-Fname="' . $row["ComplainantFName"] . '" data-request-Mname="' . $row["ComplainantMName"] . '" data-request-Lname="' . $row["ComplainantLName"] . '" data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '">
            <a>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<p class="text-lg font-semibold text-center">Set Setteled  Report</p>
</div>
...
<div class="hidden">
<label for="Adminactiondone" class="block text-sm font-medium text-gray-700">action-Done</label>
<input value="Mark as Setteled Report ${ID}"  type="text" name="Adminactiondone" id="Adminactiondone" autocomplete="street-address" 
  class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 mb-4">
</div>
<div class="hidden">
<label for="categorization" class="block text-sm font-medium text-gray-700">categorization</label>
...
echo '<script>
$(function() {
$("#MarkasSetteldbtnToday").click(function(e) {
 e.preventDefault();
 var valid = $("#MarkasSetteleddresidentreqToday")[0].checkValidity();
 if (valid) {
     var formData = new FormData($("#MarkasSetteleddresidentreqToday")[0]);
     $.ajax({
         type: "POST",
         url: "../Backend/Update.php",
         data: formData,
...
document.getElementById("MarkasSetteldbtnToday").addEventListener("click", function() {
var dialog = document.getElementById("MarkasSetteledToday");
dialog.close();
});
</script>';
