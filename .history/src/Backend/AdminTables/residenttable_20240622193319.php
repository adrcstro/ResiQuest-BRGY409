...
$male_query = "SELECT COUNT(*) as male_count FROM users INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID AND users.Gender = 'Male'";
$male_result = mysqli_query($conn, $male_query);
$male_row = mysqli_fetch_assoc($male_result);
$male_count = $male_row['male_count'];


// Count the number of female residents
$female_query = "SELECT COUNT(*) as female_count FROM users INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID AND users.Gender = 'Female'";
$female_result = mysqli_query($conn, $female_query);
$female_row = mysqli_fetch_assoc($female_result);
$female_count = $female_row['female_count'];

$total_query = "SELECT COUNT(*) as total_count FROM users INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID";
$total_result = mysqli_query($conn, $total_query);
$total_row = mysqli_fetch_assoc($total_result);
...
if (mysqli_num_rows($result) > 0) {
    ...
    <ol class="items-center justify-center gap-6 sm:flex flex-wrap">
