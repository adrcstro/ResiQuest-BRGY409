  ...
  echo '<td class="p-3 border border-inherit text-center   whitespace-nowrap text-sm font-semibold text-gray-900">
    <div class="dropdown dropdown-end">
        ...
        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
            <li class="setscheduleBlotterOverdue" data-setscheduleblootter-id="' . $row["ComplainID"] . '" data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"] . ' " data-adminposition-logs="' . $row["BrgyPosition"] . '"
            >
            <a>
             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
echo '</tbody></table></div>';
echo '<dialog id="setblotterscheduleOverdue" class="modal p-3 ">
<div class="modal-box w-70 max-w-3xl rounded-lg">
    ...
    <form id="setscheduleBlotterformOverdue" action="../Backend/Update.php" method="post" enctype="multipart/form-data">
  <div id="setscheduleBlottermodalcontentOverdue"></div>
  </form>
  <div class="mt-4 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
  <button id="setscheduleBlottercloseOverdue" class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">
  ✕
</button>
<div class="px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
<button" id="setscheduleBlotterbuttonOverdue" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
...
const modal = document.getElementById("setblotterscheduleOverdue");
const modalContent = document.getElementById("setscheduleBlottermodalcontentOverdue");
const openModalButtons = document.querySelectorAll(".setscheduleBlotterOverdue");
const closeModalButton = document.getElementById("setscheduleBlottercloseOverdue");
...
<input value="Reschedule Overdue Case ${setscheduleblotterid}"  type="text" name="Adminactiondone" id="Adminactiondone" autocomplete="street-address" 
    class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 mb-4">
</div>
<div class="hidden">
...
echo '<script>
$(function() {
  $("#setscheduleBlotterbuttonOverdue").click(function(e) {
      e.preventDefault();
      var valid = $("#setscheduleBlotterformOverdue")[0].checkValidity();
      if (valid) {
          var formData = new FormData($("#setscheduleBlotterformOverdue")[0]);
          $.ajax({
              type: "POST",
...
document.getElementById("setscheduleBlotterbuttonOverdue").addEventListener("click", function() {
  var dialog = document.getElementById("setblotterscheduleOverdue");
  dialog.close();
});
</script>';
    } else {
