...
if (!empty($_SESSION["AdminID"])) {
    ...
    $sql = "
        SELECT 
            COUNT(rd.RequestID ) AS request_count,
            COUNT(c.ComplainID) AS complaint_count,
            DATE_FORMAT(rd.DateRequested, '%Y-%m') AS month
        FROM 
            requestdocument rd
        LEFT JOIN 
            complaintbl c ON DATE_FORMAT(rd.DateRequested, '%Y-%m') = DATE_FORMAT(c.DateofReport, '%Y-%m')
        GROUP BY 
