...
if(!$conn){
  echo "Database connection error".mysqli_connect_error();
    ...
    FROM complaintbl INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID AND ComplainStatus='On-Schedule' AND (SettlementDate IS NULL OR SettlementDate = '')";
...
<div class="modal-box w-70 max-w-3xl rounded-lg">
    <div class="mb-2 lg:mb-0">
      ...
      <span class="flex text-base font-medium text-blue-500 justify-center text-center">Schedule Blotter Settlement</span>
    </div>
    <form id="setscheduleBlotterform" action="../Backend/Update.php" method="post" enctype="multipart/form-data">
  <div id="setscheduleblotter"></div>
  </form>
...
<button" id="setblotterschedbtn" class="inline-flex justify-center  rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-500 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
...
const modalContent = document.getElementById("setscheduleblotter");
...
openModalButtons.forEach(function (button) {
  button.addEventListener("click", function () {
    ...
    <p class="text-lg font-semibold text-center ">Schedule Blotter Case</p>
    </div>
...
<input value="Set Settelement Date for ${setscheduleblotterid}"  type="text" name="Adminactiondone" id="Adminactiondone" autocomplete="street-address" 
  class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 mb-4">
</div>
<div class="hidden">
...
echo '<script>
$(function() {
  $("#setblotterschedbtn").click(function(e) {
      e.preventDefault();
...
document.getElementById("setblotterschedbtn").addEventListener("click", function() {
