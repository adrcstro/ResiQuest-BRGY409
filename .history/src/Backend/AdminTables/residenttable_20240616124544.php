...
$stmt = $conn->prepare("SELECT users.ID, users.fname, users.Mname, users.lname, users.Age, users.Gender, users.Status, admintbl.Profile, admintbl.AdminID, admintbl.Lname, admintbl.Fname, admintbl.MName, admintbl.BrgyPosition 
                        FROM users 
                        INNER JOIN admintbl ON admintbl.AdminID = users.AdminID 
                        WHERE admintbl.AdminID = ? LIMIT ?, ?");
$stmt->bind_param("iii", $ID, $offset, $records_per_page);
$stmt->execute();
$result = $stmt->get_result();

$total_records_stmt = $conn->prepare("SELECT COUNT(*) as total FROM users 
                                      INNER JOIN admintbl ON admintbl.AdminID = users.AdminID 
                                      WHERE admintbl.AdminID = ?");
$total_records_stmt->bind_param("i", $ID);
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<tr>';
        echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["ID"]) . '</td>';
        echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["fname"] . ' ' . $row["Mname"] . ' ' . $row["lname"]) . '</td>';
        echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["Age"]) . '</td>';
        echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["Gender"]) . '</td>';
        echo '<td class="p-3 text-center border-2 border-inherit rounded-md text-sm font-semibold text-gray-900 ' . $statusBgColor . '">' . htmlspecialchars($row["Status"]) . '</td>';
        echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
                    ...
                    <li class="edit-residents-btn" data-residents-id="' . htmlspecialchars($row["ID"]) . '" data-residents-profile="' . htmlspecialchars($row["Profile"]) . '" data-residents-fname="' . htmlspecialchars($row["fname"]) . '" data-residents-mname="' . htmlspecialchars($row["Mname"]) . '" data-residents-lname="' . htmlspecialchars($row["lname"]) . '" data-residents-nname="' . htmlspecialchars($row["Nickname"]) . '" data-residents-placeofbirth="' . htmlspecialchars($row["PlaceofBirth"]) . '" data-residents-dateofbirth="' . htmlspecialchars($row["Dateofbirth"]) . '" data-residents-age="' . htmlspecialchars($row["Age"]) . '" data-residents-civilstat="' . htmlspecialchars($row["CivilStatus"]) . '" data-residents-gender="' . htmlspecialchars($row["Gender"]) . '" data-residents-purok="' . htmlspecialchars($row["Purok"]) . '" data-residents-voterstat="' . htmlspecialchars($row["VoterStatus"]) . '" data-residents-email="' . htmlspecialchars($row["email"]) . '" data-residents-occupation="' . htmlspecialchars($row["Occupation"]) . '" data-residents-contactnum="' . htmlspecialchars($row["ContactNumber"]) . '" data-residents-address="' . htmlspecialchars($row["Address"]) . '" data-residents-zipcode="' . htmlspecialchars($row["Zipcode"]) . '" data-residents-citizenship="' . htmlspecialchars($row["Citezenship"]) . '" data-adminid-logs="' . htmlspecialchars($row["AdminID"]) . '" data-adminusername-logs="' . htmlspecialchars($row["Lname"] . ',' . $row["Fname"] . ' ' . $row["MName"]) . '" data-adminbrgyposition-logs="' . htmlspecialchars($row["BrgyPosition"]) . '">
                        <a>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487c.524-.144 1.074.055 1.416.511l1.723 2.336a1.705 1.705 0 0 1-.04 2.066L9.522 20.25a2.118 2.118 0 0 1-.81.619l-4.81 1.91c-.574.227-1.21-.052-1.417-.607-.207-.555.03-1.186.57-1.45l4.284-2.13a2.25 2.25 0 0 0 .499-.344l10.396-10.87c.433-.452.547-1.104.282-1.663a.607.607 0 0 0-.283-.283Z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="m18.125 6.375 1.5 1.5" />
                            </svg>
                            Edit Residents
                        </a>
                    </li>
                    <li class="delete-residents-btn" data-residents-id="' . htmlspecialchars($row["ID"]) . '">
                        <a>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 7.5h12M9.75 7.5V6a2.25 2.25 0 0 1 2.25-2.25h.75a2.25 2.25 0 0 1 2.25 2.25v1.5m2.25 0v12a2.25 2.25 0 0 1-2.25 2.25H8.25A2.25 2.25 0 0 1 6 19.5V7.5h12Z" />
                            </svg>
                            Delete Residents
                        </a>
                    </li>
                    <li class="verify-residents-btn" data-residents-id="' . htmlspecialchars($row["ID"]) . '">
                        <a>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75 10.5 18l9-11.25" />
                            </svg>
                            Verify Residents
                        </a>
                    </li>
    ...
    if ($page > 1) {
        echo '<a href="?page=' . ($page - 1) . '" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Previous</a>';
    } else {
        echo '<span class="bg-gray-500 text-white font-bold py-2 px-4 rounded cursor-not-allowed">Previous</span>';
    }

    if ($page < $total_pages) {
        echo '<a href="?page=' . ($page + 1) . '" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Next</a>';
    } else {
        echo '<span class="bg-gray-500 text-white font-bold py-2 px-4 rounded cursor-not-allowed">Next</span>';
    }
    echo '</div>';
