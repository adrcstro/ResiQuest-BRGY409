...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        switch ($row["Status"]) {
            case 'Verified':
                $statusBgColor = 'bg-green-400 py-1';
                break;
            case 'NotVerified':
                $statusBgColor = 'bg-red-400 py-1';
                break;
            default:
        ...
        '<div class="flex justify-center items-center  text-white px-2 py-1 ' . $statusBgColor . ' h-full rounded">' . $row["Status"] . '</div>' . 
        '</td>';
