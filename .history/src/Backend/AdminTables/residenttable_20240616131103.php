...
// Prepare and execute query to get users and admin details
$stmt = $conn->prepare("SELECT users.*, admintbl.*
                        FROM users 
...
if (mysqli_num_rows($result) > 0) {
    ...
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
            <tr>
                <th class="bg-gray-50 border-2 border-inherit text-center px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider column-header" data-column="0">
                    Residents ID
                </th>
                <th class="bg-gray-50 px-6 py-3 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider column-header hidden" data-column="1">
                    Residents Name
                </th>
                <th class="bg-gray-50 px-6 py-3 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider column-header hidden" data-column="2">
                    Age
                </th>
                <th class="bg-gray-50 px-6 py-3 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider column-header hidden" data-column="3">
                    Gender
                </th>
                <th class="bg-gray-50 px-6 py-3 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider column-header hidden" data-column="4">
                    Account Status
                </th>
                <th class="bg-gray-100 px-6 py-3 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider column-header hidden" data-column="5">
                    Action
                </th>
            </tr>
        </thead>
   ...
   if ($page > 1) {
       echo '<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" id="prev-btn">Previous</button>';
   } else {
       echo '<button class="bg-gray-500 text-white font-bold py-2 px-4 rounded cursor-not-allowed" disabled>Previous</button>';
   }

   if ($page < $total_pages) {
       echo '<button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" id="next-btn">Next</button>';
   } else {
       echo '<button class="bg-gray-500 text-white font-bold py-2 px-4 rounded cursor-not-allowed" disabled>Next</button>';
   }
   echo '</div>';

   // JavaScript to handle column switching
   echo '
   <script>
       $(document).ready(function() {
           $(".column-header").click(function() {
               var columnIndex = $(this).data("column");
               $(".column-data").addClass("hidden");
               $(".column-data[data-column="" + columnIndex + ""]").removeClass("hidden");
           });

           $("#next-btn").click(function() {
               var nextColumn = $(".column-header:not(.hidden)").next(".column-header");
           ...
           $("#prev-btn").click(function() {
               var prevColumn = $(".column-header:not(.hidden)").prev(".column-header");
