...
// Assuming $servername, $username, $password, $dbname are defined in Connection.php
...
WHERE admintbl.AdminID = $ID LIMIT $offset, $rowsPerPage;";
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Pagination
    $query = "SELECT COUNT(*) AS total FROM users INNER JOIN admintbl ON admintbl.AdminID WHERE admintbl.AdminID = $ID";
    $result = mysqli_query($conn, $query);
    ...
    for ($i = 1; $i <= $totalPages; $i++) {
        echo '<a href="?page=' . $i . '" class="pagination-link mx-1 px-3 py-1 bg-gray-300 rounded hover:bg-gray-400" data-page="' . $i . '">' . $i . '</a>';
    }
    echo '</div>';
