...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
          ...
          View Details</a></li>
           ...
           data-Residents-address="' . $row["Address"] . '"  data-Residents-zipcode="' . $row["Zipcode"] . '"  data-Residents-citizenship="' . $row["Citezenship"] . '" data-adminid-logs="' . $row["AdminID"] . '" data-adminusername-logs="' . $row["username"] . '">
           <a>
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487a3.375 3.375 0 0 1 4.774 4.774L9.04 21.856a4.5 4.5 0 0 1-1.688 1.103l-4.147 1.44a.75.75 0 0 1-.95-.95l1.44-4.146a4.5 4.5 0 0 1 1.103-1.689L16.862 4.487Z" />
           <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 12 12 4.5" />
         </svg>
          Edit Details</a></li>
           <li class="delete-residents-btn" data-Residents-id="' . $row["ID"] . '" data-Residents-profile="' . $row["Profile"] . '"><a>
           <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
           <path stroke-linecap="round" stroke-linejoin="round" d="M6 18.75V6.75M6 18.75h12M6 18.75a1.5 1.5 0 0 0 1.5 1.5h9a1.5 1.5 0 0 0 1.5-1.5M18 6.75v12M18 6.75H6M18 6.75H6M18 6.75h-.621M18 6.75H6m0 0v-.878c0-.337.136-.659.379-.896l1.745-1.746A1.25 1.25 0 0 1 9.378 3.75h5.244a1.25 1.25 0 0 1 .88.364l1.746 1.746c.24.24.378.564.378.896v.878m-10.5 0h10.5" />
         </svg>
           Delete Details</a></li>
       </ul>
     </div>
    ...
    if ($total_pages > 1) {
        if ($page > 1) {
            echo '<a href="#" class="pagination-btn px-3 py-2 mx-1 bg-gray-200 rounded" data-page="' . ($page - 1) . '">Previous</a>';
        }

        for ($i = 1; $i <= $total_pages; $i++) {
            if ($i == $page) {
                echo '<a href="#" class="pagination-btn px-3 py-2 mx-1 bg-blue-500 text-white rounded" data-page="' . $i . '">' . $i . '</a>';
            } else {
                echo '<a href="#" class="pagination-btn px-3 py-2 mx-1 bg-gray-200 rounded" data-page="' . $i . '">' . $i . '</a>';
            }
        }

        if ($page < $total_pages) {
            echo '<a href="#" class="pagination-btn px-3 py-2 mx-1 bg-gray-200 rounded" data-page="' . ($page + 1) . '">Next</a>';
        }
    }
    echo '</nav>
</div></div>';
