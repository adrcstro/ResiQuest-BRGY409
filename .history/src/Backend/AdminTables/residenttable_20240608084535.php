...
function fetchData($conn, $ID, $records_per_page, $offset) {
...
if (isset($_GET['ajax'])) {
    $result = fetchData($conn, $ID, $records_per_page, $offset);
    $data = [];
...
<body>
    ...
    <div class="flex justify-center mt-4 pagination">
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="?page=<?= $i ?>" class="mx-1 px-3 py-1 border border-blue-500 bg-white text-blue-500 rounded" data-page="<?= $i ?>"><?= $i ?></a>
        <?php endfor; ?>
    </div>
</body>
</html>
