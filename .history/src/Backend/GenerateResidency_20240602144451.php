...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->SetX(150); // Set X coordinate to 30 (adjust as needed)
...
$this->Line(10, 48, 200, 48);
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT requestdocument.*, admintbl.*
              ...
              WHERE RequestID = '$searchID' AND ServiceType = 'Certificate of Residency' AND BrgyPosition='Barangay Chairman'";
        ...
        if ($result) {
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    $pdf->MultiCell(0, 10, $letterContent, 0, 'J');
                    $pdf->SetY($pdf->GetY() + 10);
                    $pdf->SetX(20);
                    $fullName = $row['Fname'] . ' ' . $row['MName'] . ' ' . $row['Lname'];
$pdf->SetX(30); // Set X coordinate to 30 (adjust as needed)
$pdf->SetFont('times', 'B', 12); // Set font to bold
$pdf->Cell(0, -5, $fullName, 0, 10, 'R');
$pdf->SetFont('times', '', 12); // Reset font to normal after the cell is drawn
