...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT RequestID, ServiceType, DateRequested, FirstName, Middlename, Lastname, Address,	BusinessAct,BusinessLoc  FROM requestdocument WHERE RequestID = '$searchID' AND (ServiceType = 'Barangay Identification')";
...
_____/ Complying with the provitions of existing Barangay ordinances, rules and regualtions being enforced in this barangay:   
_____/ Partially complying with the provision of existing Barangay ordinances rules and regulation being enforced in this barangay: 
             In view of the foregoing, this Barnagay thru the undersign ---
_____/ Interposes NO objection for the issuance of the corresponding Mayor's Permit being applied for:
_____/ Recommend only the issuance of TEMPORARY PERMIT for not more than (3) months and within the period of the requirments under existing barangay ordinances rules and regulation on that matter should be totally compiled with otherwise this barangay would take the necessary action,within legal bounds to stop its operation:";
