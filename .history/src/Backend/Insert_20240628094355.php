...
if (isset($_POST['ResidentIDMessage'], $_POST['chat'], $_POST['date'], $_POST['time'], $_POST['SenderFullname'])) {
    ...
    $sql = "INSERT INTO messages (MessageID, SenderID,SenderProfile,SenderName, msg, Date, Time, Profile) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssssssss',  $MessageID, $ResidentIDMessage,$PRofileMEssagesenderFile, $SenderFullname, $chat, $date, $time,$imageuploadMessageFile,$PRofileMEssagesenderFile);
    if ($stmt->execute()) {
        move_uploaded_file( $imageuploadMessageTmp, "uploads/" .$imageuploadMessageFile);
        move_uploaded_file( $PRofileMEssagesenderTmp, "uploads/" .$PRofileMEssagesenderFile);
