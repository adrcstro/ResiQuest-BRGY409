...
class PDF extends TCPDF {
    ...
    public function __construct($organizationName, $organizationAddress, $brgyname, $Zonenumname, $fulladdress , $officeofbrgy, $serviceType, $nameofbrgycharman, $psotiontitle  ) {
        parent::__construct();
...
$midpoint = $this->GetPageWidth() / 2;

// Get the width of each cell
$nameWidth = $this->GetStringWidth($this->nameofbrgycharman);
$titleWidth = $this->GetStringWidth($this->psotiontitle);

// Calculate the x position for nameofbrgycharman to be aligned to the right
$xName = $midpoint - ($nameWidth + 10); // 10 is the padding or distance between cells

// Calculate the x position for psotiontitle to be aligned to the right
$xTitle = $midpoint + 10; // 10 is the padding or distance between cells

// Set the margins and line spacing as needed
$this->SetMargins(30, 25, 30);
$this->Ln(120); // Adjust the line spacing as needed

// Set font for nameofbrgycharman
$this->SetFont('times', 'B', 12);
$this->Cell(0, 10, $this->nameofbrgycharman, 0, 9, 'R');
$this->Ln(-5);

// Set font for psotiontitle
$this->SetFont('times', 'B', 10);
$this->Cell($xTitle - $midpoint, 10, '', 0); // Add empty cell to align with nameofbrgycharman
$this->Cell(0, 10, $this->psotiontitle, 0, 9, 'R');
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf = new PDF($organizationName, $organizationAddress ,$brgyname, $Zonenumname, $fulladdress, $officeofbrgy, $serviceType, $nameofbrgycharman, $psotiontitle);
