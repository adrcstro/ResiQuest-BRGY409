...
if (isset($_POST['AdminsID'])) {
    ...
    if (isset($_FILES['AdminPRofile']) && !empty($_FILES['AdminPRofile']['name'])) {
        $AdminPRofiles = $_FILES['AdminPRofile']['name'];
        move_uploaded_file($_FILES['AdminPRofile']['tmp_name'], "uploads/" . $AdminPRofiles);
    } else {
        $AdminPRofiles = $existingAdminProfile;
    }
    ...
    if (!empty($adminID)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>Swal.fire('Error', 'Error Updating Record: {$stmt->error}', 'error');</script>";
        }

        $stmt->close();
    } else {
        echo "<script>Swal.fire('Error', 'Admin ID not provided.', 'error');</script>";
    }
}
