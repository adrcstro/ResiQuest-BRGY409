...
if (isset($_POST['ResidentIDMessage'], $_POST['chat'], $_POST['date'], $_POST['time'], $_POST['SenderFullname'])) {
    ...
    $PRofileMEssagesenderFile = $_FILES['PRofileMEssagesender']['name'];
    $PRofileMEssagesenderTmp = $_FILES['PRofileMEssagesender']['tmp_name'];

    $MessageID = generateUniqueRandomNumber($conn, 'messages', 'MessageID');
    $sql = "INSERT INTO messages (MessageID, SenderID, SenderProfile, SenderName, msg, Date, Time, MessageImage) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssssssss', $MessageID, $ResidentIDMessage, $PRofileMEssagesenderFile, $SenderFullname, $chat, $date, $time, $imageuploadMessageFile);
    
    if ($stmt->execute()) {
        if (move_uploaded_file($imageuploadMessageTmp, "uploads/" . $imageuploadMessageFile) && move_uploaded_file($PRofileMEssagesenderTmp, "uploads/" . $PRofileMEssagesenderFile)) {
