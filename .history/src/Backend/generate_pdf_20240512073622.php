...
$organizationAddress = "Barangay 409";
// Create connection
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($serviceType == "Certificate of Residency" || $serviceType == "Certificate of Indigency") {
        ...
        // If the ServiceType does not match any of the above, set an empty result
        $sql = "SELECT NULL WHERE 1 = 0"; // Empty query that returns no results
    }
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Append specific details based on ServiceType
            if ($serviceType == "Certificate of Residency" || $serviceType == "Certificate of Indigency") {
                $letterContent .= "
                    Purpose: {$row['Purpose']}
                ";
                ...
                $letterContent .= "
                    Business Activity: {$row['BusinessAct']}
                    Business Location: {$row['BusinessLoc']}
                ";
            } elseif ($serviceType == "Barangay Identification") {
                $letterContent .= "
