...
if (isset($_POST['ResidentID'])) {
    ...
    $sql = "SELECT Profile, GovernmentIDFront FROM users WHERE ID = ?";
    ...
    $GovernmentIDBack = !empty($_FILES['Government-ID-back']['name']) ? $_FILES['Government-ID-back']['name'] : $existingGovernmentID;
    ...
    if (!empty($ResidentID)) {
        ...
        $stmt->bind_param("sssssssssssssssssssss", $firstname, $Middlename, $lastname, $ContactNumber, $Gender, $email, $Nickname, $PlaceofBirth, $Birthdate, $Age, $civilstatus, $purok, $voterstatus, $Occupation, $Address, $ZipCode, $Citizenship, $PRofile, $GovernmentID, $ResidentID);
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>Swal.fire('Success', 'Application submitted successfully', 'success').then(() => { window.location = '../Frontend/ResidentsDashboard.php'; });</script>";
        } else {
            echo "<script>Swal.fire('Error', 'Error updating driver record: {$stmt->error}', 'error');</script>";
        }
    
        $stmt->close();
    } else {
        echo "<script>Swal.fire('Warning', 'Please select a driver to update', 'warning');</script>";
    }
