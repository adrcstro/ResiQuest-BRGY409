...
if (isset($_POST['NewsTitle'], $_POST['NewsDate'], $_POST['NewsTime'], $_POST['NewsContent'])) {
    $NewsContent = $_POST['ResidentIDMessage'];
    $NewsTitle = $_POST['chat'];
    $NewsDate = $_POST['date'];
    $NewsTime = $_POST['time'];
    $NewsImageFile = $_FILES['image-upload-Message']['name'];
    $NewsImageTmp = $_FILES['image-upload-Message']['tmp_name'];

    $MessageID= generateUniqueRandomNumber($conn, 'newsandevents', 'MessageID');
    $sql = "INSERT INTO newsandevents (NewsID, NewsTittle, NewsContent, Newstime, NewsDate, NewsImages) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssssss', $Newsid, $NewsTitle, $NewsContent,$NewsTime,$NewsDate,$NewsImageFile);
    if ($stmt->execute()) {
