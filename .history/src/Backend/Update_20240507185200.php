...
if (isset($_POST['AdminsID'])) {
    ...
    if (!empty($adminID)) {
        $stmt = $conn->prepare("UPDATE admintbl SET Fname=?, MName=?, Lname=?, BrgyPosition=?, AdGender=?, Email=?, AdminProfile=? WHERE AdminID=?");
...
if (isset($_POST['adminedituserid'])) {
    $ResidentID = $_POST['adminedituserid'];
    ...
    $firstname = $_POST['AdminEditresidentFirstName'];
    $Middlename = $_POST['AdminEditresidentMiddleName'];
    $lastname = $_POST['AdminEditresidentLastName'];
    $Nickname = $_POST['AdminEditresidentnickname'];
    $PlaceofBirth = $_POST['AdminEditresidentplaceofbirth'];
    $Birthdate = $_POST['AdminEditresibirthdate'];
    $Age = $_POST['AdminEditresiage'];
    $civilstatus = $_POST['AdminEditresicivilstat'];
    $Gender = $_POST['AdminEditGender'];
    $purok = $_POST['admineditresipurok'];
    $voterstatus = $_POST['Admineditresivotestats'];
    $email = $_POST['Admineditresiemail'];
    $Occupation = $_POST['Admineditresioccupation'];
    $ContactNumber = $_POST['Admineditresicontact'];
    $Address = $_POST['Admineditresiaddress'];
    $ZipCode = $_POST['Admineditresizipcode'];
    $Citizenship = $_POST['Admineditresicitizenship'];

    $PRofile = !empty($_FILES['admineditresiPRofile']['name']) ? $_FILES['admineditresiPRofile']['name'] : $existingProfile;
    ...
    if (!empty($ResidentID)) {
        ...
        $stmt->bind_param("sssssssssssssssssssss", $firstname, $Middlename, $lastname, $ContactNumber, $Gender, $email, $Nickname, $PlaceofBirth, $Birthdate, $Age, $civilstatus, $purok, $voterstatus, $Occupation, $Address, $ZipCode, $Citizenship, $PRofile, $ResidentID);
    
        if ($stmt->execute()) {
            // Move uploaded files to the specified directory
            if (!empty($_FILES['admineditresiPRofile']['tmp_name'])) {
                move_uploaded_file($_FILES['admineditresiPRofile']['tmp_name'], "uploads/" . $PRofile);
            }
