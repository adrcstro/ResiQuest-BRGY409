...
while($row = mysqli_fetch_assoc($result)) {
  ...
  echo '<div class="grid justify-start grid-cols-1 md:grid-cols-2 gap-4">';
  foreach ($row as $key => $value) {
      echo '<div class="text-left">';
      echo '<label for="' . $key . '" class="block text-sm font-semibold leading-6 text-gray-900">' . $key . '</label>';
      echo '<input type="text" id="' . $key . '" name="' . $key . '" value="' . $value . '" readonly class="block w-full mt-1 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">';
      echo '</div>';
  }
  echo '</div>';
}
