...
if (!empty($_SESSION["ID"]) && !empty($status)) {
    ...
    echo '<div class="flex space-x-6  justify-center ">'; // Start flex container
    foreach ($statusCounts as $statusKey => $count) {
        echo '
        <span class="text-sm font-medium text-gray-700 mb-2">' . $statusKey . ' = ' . $count . '
        </span>';
    }
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                if ($status == 'On-Schedule') {
                    echo '<td class="px-6 py-4 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["SettlementDate"] . ' | ' . $row["SettlementTime"] . ' </td>';
         
                } else if ($status == 'DateSettled') {
                    echo '<td class="px-6 py-4 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["DateSettled"] . '</td>';
                }
                echo '<td class="p-3 border-2 border-inherit text-center text-sm font-semibold text-gray-900">' . 
        '<div class="flex justify-center items-center  text-white px-2 py-2 ' . $statusBgColor . ' h-full rounded">' . $row["ComplainStatus"] . '</div>' . 
        '</td>';
                echo '</tr>';
            }
        } else {
