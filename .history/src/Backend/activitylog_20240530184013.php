...
if (!empty($_SESSION["AdminID"]) && !empty($status)) {
    ...
    $statusCounts = [];
    $statusCounts['process'] = getCountByStatus($conn, $ID, 'Pending');
    $statusCounts['Deleted'] = getCountByStatus($conn, $ID, 'On-Process');
    $statusCounts['Edit'] = getCountByStatus($conn, $ID, 'On-Schedule');
    $statusCounts['set'] = getCountByStatus($conn, $ID, 'Settled');
