...
if (isset($_POST['ResidentID'])) {
    ...
    if (!empty($ResidentID)) {
        $stmt = $conn->prepare("UPDATE users SET fname=?, Mname=?, lname=?, Gender=?, email=?, Nickname=?, PlaceofBirth=?, Dateofbirth=? , Age=? , CivilStatus=? , Purok=? , VoterStatus=? , Occupation=? , Address=? , Zipcode=? , Citezenship=? ,Profile=? , GovernmentID=?   WHERE ID=?");
