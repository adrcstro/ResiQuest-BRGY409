...
// Split the paragraph text into an array of lines
$lines = explode("\n", $paragraphText);

// Loop through each line except the last one
for ($i = 0; $i < count($lines) - 1; $i++) {
    $pdf->MultiCell(0, 10, $lines[$i], 0, 'J');
    $pdf->SetX(30); // Reset X coordinate after each MultiCell
}

// Handle the last line separately to prevent justification issues
$lastLine = end($lines);
$pdf->Cell(0, 10, $lastLine, 0, 1, 'L');

// Adjust the spacing after the content
$pdf->SetY($pdf->GetY() + 10); // Increase the value (10) for more spacing
 // Increase the value (10) for more spacing
}
