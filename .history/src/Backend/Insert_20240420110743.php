...
if (isset($_POST['firstnamebrgyResidency'], $_POST['MnamebrgyResidency'], $_POST['lastnamebrgyResidency'], $_POST['DaterequestedbrgyResidency'], $_POST['AddressdbrgyResidency'], $_POST['PurposebrgyResidency'], $_POST['ServicetypebrgyResidency'])) {
    ...
    // Concatenate first name, middle name, and last name into a single string
    $ResidentsName = $firstnamebrgyResidency . ' ' . $MnamebrgyResidency . ' ' . $lastnamebrgyResidency;

    // Prepare and bind for insertion
    $stmt = $conn->prepare("INSERT INTO requestdocument (ResidentsName, ServicetypebrgyResidency, DaterequestedbrgyResidency, AddressdbrgyResidency) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $ResidentsName, $ServicetypebrgyResidency, $DaterequestedbrgyResidency, $AddressdbrgyResidency);
