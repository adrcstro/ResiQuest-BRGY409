...
if (isset($_POST['AdminsID'])) {
    require_once 'Connection.php'; // Include your database connection settings

    $adminID = $_POST['AdminsID'];
    $stmt = $conn->prepare("SELECT AdminProfile FROM admintbl WHERE AdminID=?");
    ...
    if (!empty($adminID)) {
        ...
        if ($stmt->execute()) {
            if (!empty($_FILES['AdminPRofile']['tmp_name'])) {
                $uploadDir = "uploads/";
                $uploadFile = $uploadDir . basename($_FILES['AdminPRofile']['name']);
                if (move_uploaded_file($_FILES['AdminPRofile']['tmp_name'], $uploadFile)) {
