...
if(mysqli_num_rows($result) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo '<label for="status" class="block text-sm font-semibold leading-6 text-gray-900">Status</label>';
    echo '<div class="mt-2.5">';
    echo '<input type="text" id="status" name="status" value="' . $row["fname"] . '" readonly class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">';
    echo '</div>';
    echo '</div>';
