...
if (isset($_POST['AdminsID'])) {
    ...
    if (!empty($adminID)) {
        $stmt = $conn->prepare("UPDATE admintbl SET Fname=?, MName=?, Lname=?, BrgyPosition=?, AdGender=?, Email=? WHERE AdminID=?");
        $stmt->bind_param("ssssssss", $AdminEditFirstName,  $AdminEditMiddleName,  $AdminEditLastName, $BrgyPositions, $AdminGender, $AdminEmail, $adminID);
