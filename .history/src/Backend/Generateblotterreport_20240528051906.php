...
$serviceType = "BARANGAY IDENTIFICATION FORM";
$persontocontact = "PERSON TO CONTACT IN CASE OF EMERGENCY";
...
class PDF extends TCPDF {
    ...
    public function __construct($organizationName, $organizationAddress, $brgyname, $Zonenumname, $fulladdress , $officeofbrgy, $serviceType ,$tel,$persontocontact )
    ...
    public function Header() {
        ...
        $this->Ln(85); // Adjust the line spacing as needed
        $this->SetFont('times', 'B', 14); // Set font to bold and size 14
        $this->Cell(0, 10, $this->persontocontact, 0, 9, 'C');
$this->SetLineWidth(1);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $searchID = $_POST["blotterIDprint"];

    // Fetch data from the database based on the search ID
    $sql = "SELECT RequestID, ServiceType, DateRequested, FirstName, Middlename, Lastname, Address,DateofBirth,	PlaceofBirth,Civilstat,SSSGSIS,RequestorContactNum ,Contactname, Contactaddress,Contactnum FROM requestdocument WHERE RequestID = '$searchID' AND (ServiceType = 'Barangay Identification')";

    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Create PDF instance with organization details
        $pdf = new PDF($organizationName, $organizationAddress ,$brgyname, $Zonenumname, $fulladdress, $officeofbrgy, $serviceType,$tel,$persontocontact);
...
while ($row = $result->fetch_assoc()) {
    ...
    $dateOfReport = "PERSONAL DETAILS";
    $letterContent .= str_pad($dateOfReport, 150, " ", STR_PAD_RIGHT) . "\n"; // Increased spacing for the date
    
    $letterContent .= "NAME: " . $row['Lastname'] . ", " . $row['FirstName'] . "  " . $row['Middlename'] . "\n";
    $letterContent .= "ADDRESS: " . $row['Address'] . "\n";
    $letterContent .= "DATE OF BIRTH: " . $row['DateofBirth'] . "\n";
    $letterContent .= "PLACE OF BIRTH: " . $row['PlaceofBirth'] . "\n";
    $letterContent .= "STATUS: " . $row['Civilstat'] . "    SSS/GSIS: " . $row['SSSGSIS'] . "\n";
    $letterContent .= "CONTACT NUMBER: " . $row['RequestorContactNum'] . "\n\n\n\n\n";
    
    $letterContent .= "NAME: " . $row['Contactname'] . "\n";
    $letterContent .= "ADDRESS: " . $row['Contactaddress'] . "\n";
    $letterContent .= "CONTACT NUMBER: " . $row['Contactnum'] . "\n";
    
    $pdf->MultiCell(0, 12, $letterContent, 0, 'L');
