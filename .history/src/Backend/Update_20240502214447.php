...
if (isset($_POST['AdminsID'])) {
    ...
    $BrgyPosition = $_POST['AdminPosition'];
    $AdminPRofile = !empty($_FILES['AdminPRofile']['name']) ? $_FILES['AdminPRofile']['name'] : $existingAdminProfile;



    if (!empty($adminID)) {
        $stmt = $conn->prepare("UPDATE admintbl SET Fname=?, MName=?, Lname=?, Gender=?, Email=?, BrgyPosition=?, AdminProfile=? WHERE AdminID=?");
        $stmt->bind_param("ssssssss", $AdminEditFirstName,  $AdminEditMiddleName,  $AdminEditLastName, $AdminGender, $AdminEmail, $BrgyPosition, $AdminPRofile, $adminID);
