...
if (isset($_POST['blotterprocessID'])) {
    ...
    if (!empty($markreleasedID)) {
        $stmt = $conn->prepare("UPDATE complaintbl SET ComplainStatus=? WHERE ComplainID =?");
        $stmt->bind_param("ss", $releasedstat, $markreleasedID);
        if ($stmt->execute()) {
...
if (isset($_POST['completedblotterID'])) {
    $markreleasedID= $_POST['completedblotterID'];
    $releasedstat = $_POST['blottercompletestat'];
    if (!empty($markreleasedID)) {
        $stmt = $conn->prepare("UPDATE complaintbl SET ComplainStatus=? WHERE ComplainID =?");
        $stmt->bind_param("ss", $releasedstat, $markreleasedID);
        if ($stmt->execute()) {
            echo 'Mark Completed Successfully';
...
if (isset($_POST['SettledID'])) {
    $markreleasedID= $_POST['SettledID'];
    $releasedstat = $_POST['Settleddstat'];
    if (!empty($markreleasedID)) {
        $stmt = $conn->prepare("UPDATE complaintbl SET ComplainStatus=? WHERE ComplainID =?");
        $stmt->bind_param("ss", $releasedstat, $markreleasedID);
        if ($stmt->execute()) {
            echo 'Mark Settled Successfully';
...
if (isset($_POST['blotteridsetschedID'])) {
    $scheduleID= $_POST['blotteridsetschedID'];

    $Setdate = $_POST['setdateblotter'];
    $Settime  = $_POST['settimeblotter'];


    if (!empty($scheduleID)) {
        $stmt = $conn->prepare("UPDATE complaintbl SET SettlementDate=?, SettlementTime=? WHERE ComplainID=?");
        $stmt->bind_param("sss",$Setdate,$Settime, $scheduleID);
        if ($stmt->execute()) {
            echo 'Blotter Schedule Set Successfully';
