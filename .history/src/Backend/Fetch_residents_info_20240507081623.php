...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    if (!empty($row['Profile']) && file_exists($profileImagePath)) {
        ...
        // Display default image from the "Images" folder if profile image does not exist
        echo '<img id="profilePicture" src="/Images/blank.png" class="w-32 h-32 bg-gray-300 rounded-full mb-2 border-4" alt="Default Profile Picture">';
    }
  ...
  echo '<div class="flex items-center flex-col md:flex-row">';
  echo '<label for="resident-id" class="font-semibold mr-2">Residents ID:</label>';
  echo '<h1 id="resident-id" class="text-lg mb-2 md:mb-0">' . $row["ID"] . '</h1>';
  echo '<span class="mx-4 hidden md:inline">|</span>';
