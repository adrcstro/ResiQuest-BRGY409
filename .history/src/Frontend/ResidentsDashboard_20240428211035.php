...
if (!empty($_SESSION["ID"])) {
  ...
  // Prepare and execute the SQL query to fetch requests for the user
  $query = "SELECT * FROM requestdocument WHERE UserID = $ID";
  $result = mysqli_query($conn, $query);
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
         ...
         View Details</a></li>
          <li class="edit-request-btn" data-request-id="' . $row["RequestID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    // Output the edit modal
    echo '<div id="edit-request-modal" class="modal">
            <div class="modal-box">
              <h2>Edit Request</h2>
              <form action="update_request.php" method="post">
                <input type="hidden" name="request-id" id="edit-request-id">
                <label for="edit-first-name">First Name:</label>
                <input type="text" name="edit-first-name" id="edit-first-name" required>
                <label for="edit-middle-name">Middle Name:</label>
                <input type="text" name="edit-middle-name" id="edit-middle-name" required>
                <label for="edit-last-name">Last Name:</label>
                <input type="text" name="edit-last-name" id="edit-last-name" required>
                <label for="edit-address">Address:</label>
                <input type="text" name="edit-address" id="edit-address" required>';

    // Add additional fields based on ServiceType
    switch ($row["ServiceType"]) {
      case 'Certificate of Residency':
      case 'Certificate of Indigency':
        echo '<label for="edit-purpose">Purpose:</label>
                <input type="text" name="edit-purpose" id="edit-purpose" required>';
        break;
      case 'Business Clearance':
        echo '<label for="edit-business-act">Business Activity:</label>
                <input type="text" name="edit-business-act" id="edit-business-act" required>
                <label for="edit-business-loc">Business Location:</label>
                <input type="text" name="edit-business-loc" id="edit-business-loc" required>';
        break;
      case 'Barangay Identification':
        echo '<label for="edit-date-of-birth">Date of Birth:</label>
                <input type="text" name="edit-date-of-birth" id="edit-date-of-birth" required>
                <label for="edit-place-of-birth">Place of Birth:</label>
                <input type="text" name="edit-place-of-birth" id="edit-place-of-birth" required>
                <label for="edit-contact-name">Contact Name:</label>
                <input type="text" name="edit-contact-name" id="edit-contact-name" required>
                <label for="edit-contact-address">Contact Address:</label>
                <input type="text" name="edit-contact-address" id="edit-contact-address" required>
                <label for="edit-contact-num">Contact Number:</label>
                <input type="text" name="edit-contact-num" id="edit-contact-num" required>';
        break;
      default:
    ...
    echo '<button type="submit" name="edit-submit" class="btn bg-blue-400 hover:bg-blue-500">Update Request</button>
              </form>
            </div>
          </div>';

    // Include the JavaScript for modal functionality
    echo '<script>
            document.addEventListener("DOMContentLoaded", function () {
              ...
              const editRequestIdField = document.getElementById("edit-request-id");
              const editFirstNameField = document.getElementById("edit-first-name");
              const editMiddleNameField = document.getElementById("edit-middle-name");
              const editLastNameField = document.getElementById("edit-last-name");
              const editAddressField = document.getElementById("edit-address");
              const editPurposeField = document.getElementById("edit-purpose");
              const editBusinessActField = document.getElementById("edit-business-act");
              const editBusinessLocField = document.getElementById("edit-business-loc");
              const editDateOfBirthField = document.getElementById("edit-date-of-birth");
              const editPlaceOfBirthField = document.getElementById("edit-place-of-birth");
              const editContactNameField = document.getElementById("edit-contact-name");
              const editContactAddressField = document.getElementById("edit-contact-address");
              const editContactNumField = document.getElementById("edit-contact-num");
              ...
              editRequestButtons.forEach(function (button) {
                button.addEventListener("click", function () {
                  ...
                  fetch("../Backend/fetch_request_details.php?request_id=" + requestId)
                    ...
                    .then(data => {
                      // Populate edit modal with request details for editing
                      editRequestIdField.value = data.RequestID;
                      editFirstNameField.value = data.FirstName;
                      editMiddleNameField.value = data.MiddleName;
                      editLastNameField.value = data.LastName;
                      editAddressField.value = data.Address;
                      switch (data.ServiceType) {
                        case "Certificate of Residency":
                        case "Certificate of Indigency":
                          editPurposeField.value = data.Purpose;
                          break;
                        case "Business Clearance":
                          editBusinessActField.value = data.BusinessAct;
                          editBusinessLocField.value = data.BusinessLoc;
                          break;
                        case "Barangay Identification":
                          editDateOfBirthField.value = data.DateofBirth;
                          editPlaceOfBirthField.value = data.PlaceofBirth;
                          editContactNameField.value = data.ContactName;
                          editContactAddressField.value = data.ContactAddress;
                          editContactNumField.value = data.ContactNum;
                          break;
                        default:
                          // No additional fields for other ServiceTypes
                          break;
                      }
                      // Show the edit modal
  ...
  echo '<p>Please log in to view requests.</p>';
}
