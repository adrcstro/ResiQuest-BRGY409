...
while($row = mysqli_fetch_assoc($result)) {
  ...
  if ($row["Status"] !== "Completed") {
      ...
      <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
         ...
         Edit Request</a></li>
         <li class="delete-request-btn" del-data-request-id="' . $row["RequestID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
echo '<script>
      ...
      document.addEventListener("DOMContentLoaded", function () {
          ...
          deleteRequestButtons.forEach(function (button) {
              button.addEventListener("click", function () {
                  const requestId = button.getAttribute("del-data-request-id");
