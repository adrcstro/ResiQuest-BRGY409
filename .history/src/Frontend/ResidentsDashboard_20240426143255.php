    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
          if ($row["Status"] !== "Completed") {
            ...
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900 ">' . $row["Status"] . '</td>';
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["PickUpDate"] . '</td>';
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900"><button id="viewButton" class="view-button">View</button></td>';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const dialog = document.getElementById('ViewRequestdetails');
...
<dialog id="ViewRequestdetails" class="modal p-3">
  <div class="modal-box w-full max-w-6xl rounded-none">
    <div class="bg-blue-500 shadow-2xl rounded-md p-6">
      <div class="mb-2 lg:mb-0">
        <h3 class="text-2xl font-bold text-white text-center">BRGY-409 Document Request Management System</h3>
    ...
    <div id="requestDetails" class="mt-8 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
      <!-- Request details will be dynamically inserted here -->
    </div>

    <div class=" mt-8 px-4 py-3 sm:px-6 flex align-items justify-center p-4 gap-4 flex-row">
      <button class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-500 hover:bg-red-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400  sm:w-auto sm:text-sm" onclick="document.getElementById(\'ViewRequestdetails\').close();">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-1">
          <path stroke-linecap="round" stroke-linejoin="round" d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
        </svg>
        Close
