...
if ($result->num_rows > 0) {
    ...
    foreach($messages as $senderID => $senderMessages) {
        ...
        foreach ($senderMessages as $message) {
            if ($message["isNew"]) { // Assuming there is an `isNew` flag in the database
                $newMessagesCount++;
            }
        }
        ...
        // Display notification count if there are new messages
        if ($newMessagesCount > 0) {
            echo "<div class='absolute top-0 left-0 mt-1 ml-1 bg-red-600 text-white rounded-full w-6 h-6 flex items-center justify-center'>$newMessagesCount</div>";
        }
