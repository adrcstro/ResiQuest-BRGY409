...
if ($result->num_rows > 0) {
    ...
    foreach($messages as $senderID => $senderMessages) {
        ...
        $senderProfile = !empty($senderMessages[0]["SenderProfile"]) ? htmlspecialchars($senderMessages[0]["SenderProfile"]) : '../Backend/uploads/blank.png';
        ...
        echo "<p class='font-medium text-gray-900'>" . htmlspecialchars($senderMessages[0]["SenderName"]) . "</p>";
        echo "<p class='max-w-xs text-sm text-gray-500 truncate'>Resident ID: " . htmlspecialchars($senderID) . "</p>";
        echo "<button class='mt-2 p-2 bg-blue-500 text-white rounded' onclick='openModal(" . htmlspecialchars($messagesJson) . ")'>View Message</button>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
    }
