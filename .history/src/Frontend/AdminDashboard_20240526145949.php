...
<div id="table-container" class="overflow-x-auto h-screen mt-3"></div>

<script>
function applyFilter(filter) {
    const xhr = new XMLHttpRequest();
    xhr.open("GET", window.location.href.split('?')[0] + '?filter=' + filter, true);
...
window.onload = function() {
    applyFilter(''); // Load the default filter on page load
};
</script>
...
function fetch_table_data($filter) {
    global $conn;
    $today = date('Y-m-d');
    ...
    if (mysqli_num_rows($result) > 0) {
        echo '<table class="table-auto min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
            <tr>
                <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Request ID</th>
                <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Residents Name</th>
...
if (isset($_GET['filter'])) {
    $filter = $_GET['filter'];
    fetch_table_data($filter);
     // Stop further execution since this is an AJAX request
}
?>
</div>
