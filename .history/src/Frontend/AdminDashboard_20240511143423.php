...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3 me-2.5">
  <path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" />
    ...
    <div class="flex items-center">
        ...
        <a href="#" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white view-today-schedule">View Today\'s Schedule</a>
    </div>
</li>
...
<li>
    <div class="flex items-center">
        ...
        <a href="#" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white view-overdue">View Over Due</a>
    </div>
</li>
...
<scrip><script>
document.addEventListener("DOMContentLoaded", function() {
    const todayScheduleLink = document.querySelector(".view-today-schedule");
    const overDueLink = document.querySelector(".view-overdue");
    const table = document.getElementById("request-table");

    todayScheduleLink.addEventListener("click", function(event) {
    ...
    overDueLink.addEventListener("click", function(event) {
        event.preventDefault();
        filterRequests('overdue');
    });

    function filterRequests(filterType) {
        const rows = table.querySelectorAll("tbody tr");
        rows.forEach(row => {
            const pickupDate = row.querySelector(".pickup-date").textContent;
            const currentDate = new Date().toISOString().slice(0, 10);
            if (filterType === 'today' && pickupDate !== currentDate) {
                row.style.display = "none";
            } else if (filterType === 'overdue' && pickupDate >= currentDate) {
                row.style.display = "none";
            } else {
                row.style.display = "table-row";
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            // Output table header
                            echo '<div class="overflow-x-auto h-screen"><table id="request-table" class="table-auto min-w-full divide-y divide-gray-200 mt-3">
           <thead class="bg-gray-50 ">
               <tr>
               <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Request ID</th>
               <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Residents Name</th>
