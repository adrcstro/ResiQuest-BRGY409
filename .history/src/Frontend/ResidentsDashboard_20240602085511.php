...
if ($result->num_rows > 0) {
    ...
    while ($index < count($data)) {
        ...
        for ($i = 0; $i < $numItemsInLevel && $index < count($data); $i++, $index++) {
            ...
            echo '<button class="bg-blue-400 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-2 view-details" data-details="' . htmlspecialchars(json_encode($row)) . '">';
...
<script>
    // JavaScript to handle click event on view details button
    document.querySelectorAll('.view-details').forEach(button => {
        button.addEventListener('click', function() {
            // Get the details from the button's data attribute
            const details = JSON.parse(this.getAttribute('data-details'));

            // Open modal dialog and display details
            // Here, you would write code to display the modal and populate it with the details
            console.log(details);
            // Example: Display details in console
            // You can replace this with code to display details in a modal dialog
        });
    });
</script>
