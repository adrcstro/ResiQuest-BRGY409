...
function openModal(messages) {
    ...
    // Display all messages except the latest one
    messages.slice(0, -1).forEach(message => {
        ...
        msgText.classList.add('text-sm', 'text-gray-500');
    ...
    // Display the latest message at the bottom
    const latestMessage = messages[messages.length - 1];
    const latestMessageElem = document.createElement('div');
    latestMessageElem.classList.add('mb-4');

    const latestMsgText = document.createElement('p');
    latestMsgText.textContent = latestMessage.msg;
    latestMsgText.classList.add('text-sm', 'text-gray-500');
    latestMessageElem.appendChild(latestMsgText);

    const latestMsgDate = document.createElement('p');
    latestMsgDate.textContent = latestMessage.Date + ' ' + latestMessage.Time;
    latestMsgDate.classList.add('text-xs', 'text-gray-400');
    latestMessageElem.appendChild(latestMsgDate);

    modalContent.appendChild(latestMessageElem);
