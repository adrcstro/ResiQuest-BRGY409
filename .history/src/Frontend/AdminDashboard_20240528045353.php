...
while($row = mysqli_fetch_assoc($result)) {
  ...
  echo '<td class="p-3 border border-inherit text-center   whitespace-nowrap text-sm font-semibold text-gray-900">
    <div class="dropdown dropdown-end">
        ...
        <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
            <li class="print-blotter-btn" data-Complaint-id="' . $row["ComplainID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0 1 10.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0 .229 2.523a1.125 1.125 0 0 1-1.12 1.227H7.231c-.662 0-1.18-.568-1.12-1.227L6.34 18m11.318 0h1.091A2.25 2.25 0 0 0 21 15.75V9.456c0-1.081-.768-2.015-1.837-2.175a48.055 48.055 0 0 0-1.913-.247M6.34 18H5.25A2.25 2.25 0 0 1 3 15.75V9.456c0-1.081.768-2.015 1.837-2.175a48.041 48.041 0 0 1 1.913-.247m10.5 0a48.536 48.536 0 0 0-10.5 0m10.5 0V3.375c0-.621-.504-1.125-1.125-1.125h-8.25c-.621 0-1.125.504-1.125 1.125v3.659M18 10.5h.008v.008H18V10.5Zm-3 0h.008v.008H15V10.5Z" />
          </svg>
...
//print request 
echo '<dialog id="printblotterreport" class="modal p-3 ">
<div class="modal-box w-70 max-w-3xl rounded-lg">
...
echo '<script>
document.addEventListener("DOMContentLoaded", function () {
  const modal = document.getElementById("printblotterreport");
  const modalContent = document.getElementById("printblotterdocument");
  const openModalButtons = document.querySelectorAll(".print-blotter-btn");
  const closeModalButton = document.getElementById("printblotterclose");
  const printblotterform = document.getElementById("printblotterdocsform");
  ...
  openModalButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      printblotterform.action = "../Backend/Generateblotterreport.php";
      ...
      modalContent.innerHTML = `
        <div class="flex flex-col items-center mt-3 mb-5">
          ...
          <h1 class="text-sm mt-3  font-bold"><span class="font-semibold">Request-ID:</span> ${ID}</h1>
          <div class="hidden  items-center justify-center mt-3">
            <input  value="${blotterID}"   type="text" name="RequestIDprint" id="RequestIDprint" autocomplete="given-name" placeholder=""
                class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 ">
          </div>
...
while($row = mysqli_fetch_assoc($result)) {
  ...
  echo '<td class="p-3 border border-inherit text-center  whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["ComplainantFName"] . ' ' . $row["ComplainantMName"] . ' ' . $row["ComplainantLName"] . '</td>';
  echo '<td class="p-3 border border-inherit text-center  whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["DateofReport"] . '</td>';
  echo '<td class="p-3 border border-inherit text-center  whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["Settlement-Date"] . '</td>';
  echo '<td class="p-3 border border-inherit text-center  whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["Settlement-Time"] . '</td>';
