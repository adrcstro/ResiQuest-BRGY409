...
if ($result->num_rows > 0) {
    ...
    foreach($messages as $senderID => $senderMessages) {
        ...
        usort($senderMessages, function($a, $b) {
        ...
        foreach(array_slice($senderMessages, 0, -1) as $message) {
            echo "<p class='max-w-xs text-sm text-gray-500 truncate hidden'>" . htmlspecialchars($message["msg"]) . "</p>";
            echo "<p class='text-xs text-gray-400 hidden'>" . htmlspecialchars($message["Date"]) . " " . htmlspecialchars($message["Time"]) . "</p>";
        }

        // Display the latest message at the bottom
        $latestMessage = end($senderMessages);
        echo "<p class='max-w-xs text-sm text-gray-500 truncate'>" . htmlspecialchars($latestMessage["msg"]) . "</p>";
        echo "<p class='text-xs text-gray-400'>" . htmlspecialchars($latestMessage["Date"]) . " " . htmlspecialchars($latestMessage["Time"]) . "</p>";
