...
document.addEventListener("DOMContentLoaded", function () {
  ...
  closeModalButton.addEventListener("click", function () {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            function applyFilter(filter) {
                ...
                // Make an AJAX request to get the filtered data
                fetch(url.toString(), {
                        method: 'GET',
                        headers: {
            ...
            function updateTable(data) {
                ...
                if (data.length > 0) {
                    ...
                    table.classList.add('table-auto', 'min-w-full', 'divide-y', 'divide-gray-200', 'mt-3');
                    ...
                    data.forEach(row => {
                        ...
                        tr.innerHTML = `
                            <td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">${escapeHtml(row.RequestID)}</td>
                            <td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">${escapeHtml(row.FirstName + ' ' + row.Middlename + ' ' + row.Lastname)}</td>
                            <td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">${escapeHtml(row.ServiceType)}</td>
                            <td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">${escapeHtml(row.PickUpDate)}</td>
                            <td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">${escapeHtml(row.Pickuptime)}</td>
                            <td class="p-1 border flex items-center justify-center border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">
                                <div class="dropdown dropdown-end">
                                    <div tabindex="0" role="button" class="flex items-center justify-between text-white font-semibold btn w-40 bg-blue-400 hover:bg-blue-500">
                                        Action Details
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    ...
                                    <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
                                        <li class="Schedule-resident-pickup" data-request-id="${escapeHtml(row.RequestID)}" data-request-servietype="${escapeHtml(row.ServiceType)}" data-request-Fname="${escapeHtml(row.FirstName)}" data-request-M      
                                        name="${escapeHtml(row.Middlename)}" data-request-Lname="${escapeHtml(row.Lastname)}">
                                            <a>Schedule Released</a>
                                        </li>
                                        <li class="ReSchedule-resident-request" data-request-id="${escapeHtml(row.RequestID)}" data-request-servietype="${escapeHtml(row.ServiceType)}" data-request-Fname="${escapeHtml(row.FirstName)}" data-request-Mname="${escapeHtml(row.Middlename)}" data-request-Lname="${escapeHtml(row.Lastname)}" data-reschedule-date="${escapeHtml(row.PickUpDate)}" data-reschedule-time="${escapeHtml(row.Pickuptime)}">
                                            <a>Reschedule</a>
                                        </li>
                                        <li class="mark-as-released" data-request-id="${escapeHtml(row.RequestID)}" data-request-servietype="${escapeHtml(row.ServiceType)}" data-request-Fname="${escapeHtml(row.FirstName)}" data-request-Mname="${escapeHtml(row.Middlename)}" data-request-Lname="${escapeHtml(row.Lastname)}">
                                            <a>Mark Released</a>
                                        </li>
                                    </ul>
                                </div>
                    ...
                    const noDataHtml = `
                        <div class="mx-auto text-center mt-8" style="width: 200px;">
                            ...
                            <p class="text-lg font-semibold text-center">No Request Found</p>
                        </div>
                    `;
            ...
            // Attach event listeners to the breadcrumb links
            document.querySelectorAll('a[onclick^="applyFilter"]').forEach(link => {
                link.addEventListener('click', (event) => {
                    event.preventDefault();
                    const filter = event.currentTarget.getAttribute('onclick').match(/'([^']+)'/)[1];
                    applyFilter(filter);
                });
            });
        });
    ...
    <?php
    // PHP code for handling AJAX requests and returning JSON data
