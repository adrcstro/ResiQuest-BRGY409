...
if (!empty($_SESSION["ID"])) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        // Proceed with displaying user requests or any other operations
        if (!empty($_SESSION["ID"])) {
            $ID = $_SESSION["ID"];
            $result = mysqli_query($conn, "SELECT RequestID, ServiceType, DateRequested, Status, PickUpDate 
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo '<img src="/Images/norequest.svg" alt="Illustration" style="max-width: 200px; display: block; margin: 0 auto;">';
                echo "<p class='text-lg font-semibold'>No requests found for this user.</p>";
                echo '</div>';
            }
        }
    } else {
        // User with provided ID not found
        // You can handle this case appropriately, for example, redirect to login
        header("Location: Login.php");
