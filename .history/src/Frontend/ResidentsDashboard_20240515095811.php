...
if (!empty($_SESSION["ID"])) {
    ...
    $sql = "SELECT COUNT(*) AS Released_count FROM requestdocument WHERE status = 'Released'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
...
?>
                     <button lass="relative bg-orange-300 hover:bg-orange-600 focus:outline-none focus:bg-orange-600 text-white font-bold py-2 px-4 rounded" onclick="TransactionHistory.showModal()">
