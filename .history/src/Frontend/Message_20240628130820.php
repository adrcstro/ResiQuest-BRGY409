...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<img class='object-cover w-12 h-12 rounded-lg' src='../Backend/uploads/" . htmlspecialchars($row["SenderProfile"]) . "' alt='News Image' />";
        echo "<div class='ml-3 overflow-hidden'>";
        echo "<p class='font-medium text-gray-900'>" . htmlspecialchars($row["SenderName"]) . "</p>";
        echo "<p class='max-w-xs text-sm text-gray-500 truncate'>Resident ID: " . htmlspecialchars($row["SenderID"]) . "</p>";
        echo "<p class='max-w-xs text-sm text-gray-500 truncate'>$shortMsg</p>";
        echo "<button class='text-blue-500 hover:underline' onclick='viewMessage(\"" . htmlspecialchars($row["SenderName"]) . "\", \"" . htmlspecialchars($row["SenderID"]) . "\", \"" . htmlspecialchars($row["SenderProfile"]) . "\", \"" . htmlspecialchars($row["msg"]) . "\", \"" . htmlspecialchars($row["Date"]) . "\", \"" . htmlspecialchars($row["Time"]) . "\")'>View</button>";
        echo "<p class='text-xs text-gray-400'>" . htmlspecialchars($row["Date"]) . " " . htmlspecialchars($row["Time"]) . "</p>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
    }
...
<div id="messageModal" class="fixed inset-0 z-50 hidden flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white p-4 rounded-lg max-w-md w-full">
        <div class="flex justify-between items-center">
            <h2 class="text-lg font-bold" id="modalSenderName"></h2>
            <button onclick="closeModal()" class="text-gray-500">&times;</button>
        </div>
        <div class="mt-2">
            <p id="modalSenderID" class="text-sm text-gray-500"></p>
            <img id="modalSenderProfile" class="object-cover w-12 h-12 rounded-lg mt-2" src="" alt="Profile Image" />
            <p id="modalMessage" class="text-sm text-gray-900 mt-2"></p>
            <p id="modalDateTime" class="text-xs text-gray-400 mt-2"></p>
        </div>
    </div>
</div>

<script>
function viewMessage(senderName, senderID, senderProfile, msg, date, time) {
    document.getElementById('modalSenderName').innerText = senderName;
    document.getElementById('modalSenderID').innerText = "Resident ID: " + senderID;
    document.getElementById('modalSenderProfile').src = '../Backend/uploads/' + senderProfile;
