...
while($row = mysqli_fetch_assoc($result)) {
  ...
  if ($row["Status"] !== "Completed" && $row["Status"] !== "Released") {
    ...
    if ($row["Status"] === "On-Process" || $row["Status"] === "Completed") {
        echo '<script>
                Swal.fire({
                  icon: "error",
                  title: "Cannot Edit Request",
                  text: "The barangay is currently processing your request.",
                  confirmButtonText: "OK"
                });
              </script>';
    } else {
        echo '<li class="open-modal-btn" data-request-id="' . $row["RequestID"] . '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
