  ...
  <div class="modal-box w-full max-w-6xl rounded-none">
    ...
    <div class="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
      ...
      <form id="updateForm" action="../Backend/Update.php" method="post" enctype="multipart/form-data">
                               ...
                               <div class="relative">
                               <input type="file" id="PRofile" name="PRofile" class="flex" accept=".jpg, .png .jpeg .pdf, .doc, .docx" onchange="displaySelectedFile(event)">

<script>
function displaySelectedFile(event) {
    const input = event.target;
    const fileName = input.files[0].name; // Get the name of the selected file
    const displayInput = document.getElementById('displayFileName');
    displayInput.value = fileName; // Set the file name to another input field for display
}
</script>

<!-- Input field to display the file name -->
<input value="<?php echo $row["Profile"]?>" type="text" id="displayFileName" class="flex" readonly>
