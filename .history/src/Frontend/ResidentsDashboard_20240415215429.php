...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result) {
        ...
        if (password_verify($password, $hashed_password)) {
            // Passwords match, open SweetAlert2 success message
            echo "<script>
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
            ...
            // Passwords do not match, open SweetAlert2 error message
            echo "<script>
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
        ...
        // Error querying the database, open SweetAlert2 error message
        echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Database query error: " . mysqli_error($conn) . "'
                });
            </script>";
    }
...
<dialog id="viewid" class="modal">
    <div class="modal-box">
        <form method="post" action="../Backend/check_password.php">
            <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
            
            <h3 class="font-bold text-lg">Enter Account Password</h3>
        
            <div>
                <div class="mt-2.5">
                    <input value="<?php echo $row["ID"]?>" required="" type="text" name="user_id" id="user_id" autocomplete="given-name"
                                         placeholder="Residents ID" 
                                         class="w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 hidden">
            ...
            <div class="mt-3 flex items-center justify-center">
                <button type="submit" class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-400 hover:bg-green-600 text-base font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 sm:w-auto sm:text-sm"> 
