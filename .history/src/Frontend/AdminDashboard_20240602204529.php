...
$status = $row['BrgyPosition'];

// Check if status is 'NotVerified'
if ($status == 'Barangay Chairman') {
    // Hide the navigation item
    echo '<style>#hideactivitylog { display: block; }</style>';
}else{
    echo '<style>#hideactivitylog { display: none; }</style>';
}
    }else{
...
<div>
    <nav class="bg-white border-b border-gray-200 fixed z-30 w-full">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-5 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
        ...
        <button id="hideactivitylog" onclick="viewactivitylog.showModal()" type="button" class="relative rounded-full p-1 text-gray-500 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2">
