...
document.getElementById('printanalytics').addEventListener('click', function() {
    ...
    if (radioValue === 'DocumentReq') {
        window.location.href = `../Backend/generate_monthly_request_chart.php?year=${selectedYear}&month=${selectedMonth}`;
    } else if (radioValue === 'ComplaintRep') {
        window.location.href = `../Backend/generate_monthly_report_chart.php?year=${selectedYear}&month=${selectedMonth}`;
    }
});
</script>
