...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<h2 class='font-bold text-lg'>" . htmlspecialchars($row["NewsTittle"]) . "</h2>";
        echo "<p class='py-4 text-sm md:text-base lg:text-lg'>" . htmlspecialchars($row["NewsContent"]) . "</p>";
        echo "<p class='py-1 text-sm text-gray-400'>" . htmlspecialchars($row["NewsDate"]) . " " . htmlspecialchars($row["Newstime"]) . "</p>";
        echo "<img class='w-full h-64 object-cover mt-4' src='../Backend/uploads/" . htmlspecialchars($row["NewsImages"]) . "' alt='News Image' />";
