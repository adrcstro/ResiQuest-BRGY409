...
if(!empty($_SESSION["ID"])) {
    ...
    // Fetch request data from requestdocument table
    $query = "SELECT RequestID, ServiceType, DateRequested, Status 
              FROM requestdocument 
              WHERE UserID = $ID"; // Assuming UserID is the column that stores the user's ID in requestdocument table
    ...
    if($result) {
        ...
        if(mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)) {
                ...
                echo "<td class='p-4 whitespace-nowrap text-sm font-semibold text-gray-900'>"; // Assuming you want to populate this later
                echo "<td class='p-4 whitespace-nowrap text-sm font-semibold text-gray-900'>" . $row["Status"] . "</td>";
                echo "<td class='p-4 whitespace-nowrap text-sm font-semibold text-gray-900'>"; // Assuming you want to add action buttons later
                echo "</tr>";
            }
        } else {
            echo "<tr><td colspan='6' class='p-4 text-center text-gray-900'>No requests found for this user.</td></tr>";
        }
    } else {
        echo "<tr><td colspan='6' class='p-4 text-center text-gray-900'>Error: " . mysqli_error($conn) . "</td></tr>";
    }
