    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.open-resident-details-modal-btn', function() {
                var ID = $(this).data('Residents-id');

                // Fetch resident details using AJAX
                fetch('../Backend/AdminTables/Fetch_residents_info.php?residents_id=' + ID)
                    .then(response => response.text())
                    .then(data => {
                        // Display fetched details in modal
                        $('#modal-content-resident-info').html(data);
                        document.getElementById('ViewResidentsdetails').showModal();
                    });
            });
            // Close modal when clicking on the close button
            $(document).on('click', '#close-modal-btn', function() {
                document.getElementById('ViewResidentsdetails').close();
            });
        });
    </script>
</div>
