...
function applyFilter(filter) {
    ...
    xhr.open("GET", window.location.href.split('?')[0] + "?filter=" + filter, true); // Send request to the same file with the filter
    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                document.getElementById("completeschedu").innerHTML = xhr.responseText;
            } else {
