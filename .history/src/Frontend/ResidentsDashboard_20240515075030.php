...
<script>
  $(document).ready(function(){
    $("#submitcomplain").click(function(){
        ...
        var typeofcomplain = $("input[name='typeofcomplain']").val();
        var complainreportdetails = $("input[name='complainreportdetails']").val();
        // Check if any field is empty
     // Check if any field is empty
if (!complainuserid || !ComplainfirstnamebrgyID || !ComplainMnamebrgyID || !ComplainlastnamebrgyID || !Complaincontactnum || !Complaindatefiled || !Complainaddress || !complainstatus || !complaineename
|| !Complaineeaddress || !typeofcomplain || !complainreportdetails) {
    Swal.fire({
        title: 'Error!',
        text: 'Please fill out all fields for Submmiting Report Complain',
        icon: 'error',
        confirmButtonText: 'Okay',
    }).then((result) => {
        if (result.isConfirmed) {
