...
<dialog id="ViewGovernmentID" class="modal">
  <div class="modal-box w-full max-w-6xl rounded-none">
    ...
    <img src="../Backend/uploads/<?php echo $row["GovernmentIDFront"]; ?>" class="w-32 h-32 bg-gray-300 rounded-none mb-4" alt="ID">

    <img src="../Backend/uploads/<?php echo $row["GovernmentIDBack"]; ?>" class="w-32 h-32 bg-gray-300 rounded-none mb-4" alt="ID">
  </div>
</dialog>
...
<div class="bg-white shadow-2xl rounded-md p-6 mb-3  mt-4">
        <div class="mb-2 lg:mb-0">
            <h3 class="text-2xl font-bold text-black text-center">Submmision of Government ID</h3>
    ...
    <label for="Government-ID" class="flex flex-col items-center justify-center w-full h-64 p-2 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600" style="background-image: url('<?php echo isset($row["GovernmentIDFront"]) && !empty($row["GovernmentIDFront"]) ? "../Backend/uploads/".$row["GovernmentIDFront"] : "/Images/blank.png"; ?>'); background-size: 100% 100%; background-position: center;">
        ...
        <input required="" type="file" name="Government-ID" id="Government-ID" accept=".jpg, .png .jpeg .pdf, .doc, .docx" class="hidden" onchange="displayImage(this, 'front')" />
    </label>
</div>
    ...
    <div class="flex flex-col items-center justify-center w-full">
        ...
        <label for="Government-ID-back" class="flex flex-col items-center justify-center w-full h-64 p-2 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600" style="background-image: url('<?php echo isset($row["GovernmentIDBack"]) && !empty($row["GovernmentIDBack"]) ? "../Backend/uploads/".$row["GovernmentIDBack"] : "/Images/blank.png"; ?>'); background-size: 100% 100%; background-position: center;">
            ...
            <input required="" type="file" name="Government-ID-back" id="Government-ID-back" accept=".jpg, .png .jpeg .pdf, .doc, .docx" class="hidden" onchange="displayImage(this, 'back')" />
        </label>
    </div>
</div>
...
<script>
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = function(e) {
        document.getElementById('Government-ID').src = e.target.result;
      };
      reader.readAsDataURL(file);
    }
  }
  ...
  const fileInput = event.target;
  const file = fileInput.files[0];
  const profileImg = document.getElementById("Government-ID");
...
function displaySelectedImage(event) {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = function(e) {
        document.getElementById('Government-ID-back').src = e.target.result;
      };
      reader.readAsDataURL(file);
    }
  }
  ...
  const fileInput = event.target;
  const file = fileInput.files[0];
  const profileImg = document.getElementById("Government-ID-back");
...
$(document).ready(function() {
    $("#SubmitInfotoVerify").click(function(e) {
        ...
        var GovernmentIDBack = $("input[name='Government-ID-back']").prop('files')[0];
        ...
        formData.append('Government-ID-back', GovernmentIDBack);
        ...
        $.ajax({
            ...
            success: function(data) {
                Swal.fire({
                    title: 'Application for Verification Submitted Successfully',
                    icon: 'success',
