...
$(document).on('click', '.open-resident-details-modal-btn', function() {
    ...
    $.ajax({
        url: '../Backend/Fetch_residents_info.php',
        type: 'GET', // Assuming you're using GET method to fetch data
        data: { residents_id: residentId },
        success: function(response) {
            // Display fetched details in the modal
            $('#modal-content-resident-info').html(response);
            $('#ViewResidentsdetails').show(); // Show the modal
        },
        error: function() {
