...
<div  id="ResidentsBarangayOfficails"  class="absolute bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
  <div class="pt-3 px-4">
...
if ($result->num_rows > 0) {
    ...
    while ($index < count($data)) {
        ...
        echo '<div class="flex justify-center flex-wrap md:flex-nowrap">';
        for ($i = 0; $i < $numItemsInLevel && $index < count($data); $i++, $index++) {
            $row = $data[$index];
            $fullName = $row["Fname"] . " " . $row["MName"] . " " . $row["Lname"];
            echo '<div class="p-4 w-full md:w-auto">';
            ...
            echo '<button class="bg-blue-400 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-2 view-details-btn" data-id="' . $row["AdminID"] . '">';
...
<!-- Modal HTML -->
<dialog id="Admin_addresidentarchive" class="modal p-3">
    <div class="modal-box w-full  max-w-6xl rounded-none">
        <div class="flex flex-col">
            <div class="overflow-x-auto rounded-lg">
                <div class="mb-2 lg:mb-0 text-center">
                    ...
                    <span class="text-base font-medium text-blue-400">BRGY Officials Details</span>
                </div>
                <form method="dialog">
                    <button class="btn btn-sm btn-circle btn-ghost absolute top-2 right-2">✕</button>
                </form>
                <div id="modal-content" class="flex flex-col items-first">
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    const modal = document.getElementById('Admin_addresidentarchive');
    const modalContent = document.getElementById('modal-content');
    const closeModalButton = document.querySelector('form[method="dialog"] button');

    document.querySelectorAll('.view-details-btn').forEach(button => {
        button.addEventListener('click', function() {
            const adminId = this.getAttribute('data-id');
            const adminData = data.find(admin => admin.AdminID == adminId);
            
            if (adminData) {
                modalContent.innerHTML = `
                    <img src="../Backend/uploads/${adminData.AdminProfile}" alt="Avatar" class="w-24 h-24 rounded-full mx-auto mb-4 border-2 border-inherit">
                    <h2 class="text-lg font-bold  text-center">${adminData.Fname} ${adminData.MName} ${adminData.Lname}</h2>
                    <p class="font-medium text-blue-400  text-center">${adminData.BrgyPosition}</p>
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">First Name</label>
                            <input type="text" value="${adminData.Fname}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Middle Name</label>
                            <input type="text" value="${adminData.MName}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Last Name</label>
                            <input type="text" value="${adminData.Lname}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Administrator ID</label>
                            <input type="text" value="${adminData.AdminID}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Gender</label>
                            <input type="text" value="${adminData.AdGender}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Officials Position</label>
                            <input type="text" value="${adminData.BrgyPosition}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Rendered Service Start</label>
                            <input type="text" value="${adminData.RenderedServiceStart}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                        <div class="col-span-1">
                            <label class="block text-sm font-medium">Rendered Service End</label>
                            <input type="text" value="${adminData.RenderedServiceend}" class="w-full border border-gray-300 rounded-md p-2" readonly>
                        </div>
                    </div>
                `;
                modal.showModal();
