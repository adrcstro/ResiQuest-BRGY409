...
<div id="table-container" class="overflow-x-auto h-screen"></div>

<script>
function applyFilter(filter) {
    const url = new URL(window.location.href);
    url.searchParams.set('filter', filter);
    const filterUrl = url.toString();

    fetch(filterUrl)
        .then(response => response.text())
        .then(data => {
            // Parse the returned HTML and extract the table
            const parser = new DOMParser();
            const doc = parser.parseFromString(data, 'text/html');
            const tableContent = doc.querySelector('#table-container').innerHTML;
            
            // Update the table container with the new content
            document.querySelector('#table-container').innerHTML = tableContent;
        })
        .catch(error => {
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo '<table class="table-auto min-w-full divide-y divide-gray-200 mt-3">
    <thead class="bg-gray-50">
        <tr>
            <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Request ID</th>
            <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Residents Name</th>
    ...
    $table_html = ob_get_clean(); // Get the table HTML from the buffer

    if (isset($_GET['ajax'])) {
        echo $table_html; // Return only the table content if it's an AJAX request
    } else {
        echo '<div id="table-container" class="overflow-x-auto h-screen">';
        echo $table_html;
        echo '</div>';
    }
} else {
