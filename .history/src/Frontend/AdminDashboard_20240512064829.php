...
document.getElementById("rejectresidentbutton").addEventListener("click", function() {
    ...
    dialog.close();
      ...
      <li class="Print-Document-btn" data-request-id="' . $row["RequestID"] . '" data-request-servietype="' . $row["ServiceType"] . '" onclick="printDocument(' . $row["RequestID"] . ')"><a>Print Document</a></li>
      <script>
      function printDocument(requestID) {
          // AJAX request to a PHP script that generates the PDF
          $.ajax({
              type: "POST",
              url: "../Backend/generate_pdf.php",
              data: { requestID: requestID },
              success: function(response) {
                  // Assuming response is the URL of the generated PDF
