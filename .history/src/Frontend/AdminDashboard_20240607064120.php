...
if (!empty($_SESSION["AdminID"])) {
    ...
    $sql = "SELECT COUNT(*) AS total, DATE(DateofReport) AS date FROM complaintbl GROUP BY DATE(DateofReport)";
    $result = $conn->query($sql);
    $dates = [];
    $counts = [];
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            // Format the date as a month name
        ...
        scales: {
            y: {
                beginAtZero: true
                            ...
                            <h3 class="text-base font-normal text-white mt-2 mb-2">Complain Report</h3>
