...
// and the table name is 'requestdocument'
if (!empty($_SESSION["AdminID"])) {
...
// Query to count the number of "Pending" status
$query = "SELECT COUNT(*) AS pending_count FROM requestdocument WHERE status = 'Pending'";
...
<button onclick="showPendingrequest()" class="bg-red-300 hover:bg-red-600 focus:outline-none focus:bg-red-600 text-white font-bold py-2 px-4 rounded">
    <span class="absolute bg-red-500 text-white text-xs rounded-full -top-1 -left-1 px-2 py-1"><?= $pendingCount ?></span>
