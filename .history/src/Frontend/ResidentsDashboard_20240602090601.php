...
$sql = "SELECT Lname, Fname, MName, BrgyPosition, AdGender, RenderedServiceStart, RenderedServiceend, AdminProfile FROM admintbl";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while ($index < count($data)) {
        ...
        for ($i = 0; $i < $numItemsInLevel && $index < count($data); $i++, $index++) {
            ...
            echo '<button class="bg-blue-400 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-2 view-details" data-index="' . $index . '">View Details</button>';
            echo '</div>';
            echo '</div>';
        }
        echo '</div>';
...
<div id="modal" class="modal">
    <div class="modal-box">
        <div class="modal-close cursor-pointer z-50">
            <svg class="fill-current text-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                <path class="heroicon-ui" d="M6.586 6.586a2 2 0 0 1 2.828 0L12 9.172l2.586-2.586a2 2 0 1 1 2.828 2.828L14.828 12l2.586 2.586a2 2 0 1 1-2.828 2.828L12 14.828l-2.586 2.586a2 2 0 1 1-2.828-2.828L9.172 12 6.586 9.414a2 2 0 0 1 0-2.828z" />
            </svg>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                const rowData = <?php echo json_encode($data); ?>;
                const row = rowData[index];

                let detailsHtml = '';
                // Loop through the row data and display each column
                for (const key in row) {
                    if (row.hasOwnProperty(key)) {
                        detailsHtml += `<p><strong>${key}:</strong> ${row[key]}</p>`;
                    }
                }
