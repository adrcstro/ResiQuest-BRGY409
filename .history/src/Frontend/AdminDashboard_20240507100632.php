    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">
          ...
          View Details</a></li>
           ...
           <li class="edit-residents-btn"  data-Residents-id="' . $row["ID"] . '"  >
...
echo 'document.addEventListener("DOMContentLoaded", function () {
  ...
  openModalButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      // Display form in modal content
      modalContent.innerHTML = `
        <div class="flex flex-col items-center">
          <div class="relative">
            ${!empty($row["Profile"]) && file_exists("uploads/" . $row["Profile"]) ? `
              <img id="profilePicture" src="../Backend/uploads/${ID}" class="w-32 h-32 bg-gray-300 rounded-full mb-2 border-4" alt="Profile Picture">
            ` : `
              <img id="profilePicture" src="/Images/blank.png" class="w-32 h-32 bg-gray-300 rounded-full mb-2 border-4" alt="Default Profile Picture">
            `}
          </div>
          <h1 class="text-xl font-bold">${$row["fname"]}, ${$row["Mname"]} ${$row["lname"]}</h1>
          <div class="flex items-center">
            <label for="resident-id" class="font-semibold mr-2">Residents-ID:</label>
            <h1 id="resident-id" class="text-lg">${$row["ID"]}</h1>
            <span class="mx-4">|</span>
            <label for="status" class="font-semibold mr-2">Status:</label>
            <h1 class="text-lg ${$row["Status"] === "Verified" ? "text-green-500" : "text-red-500"}">${$row["Status"]}</h1>
          </div>
          <div class="mx-auto max-w-3xl mt-6">
            <div class="grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 2xl:grid-cols-3">
              <div class="flex flex-col">
              ...
              <!-- Add the rest of the input fields in a similar fashion -->
            </div>
          </div>
        </div>
      `;
