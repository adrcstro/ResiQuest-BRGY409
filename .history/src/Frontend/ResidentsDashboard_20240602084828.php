...
if ($result->num_rows > 0) {
    ...
    while ($index < count($data)) {
        ...
        for ($i = 0; $i < $numItemsInLevel && $index < count($data); $i++, $index++) {
            ...
            echo '<button class="bg-blue-400 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded mt-2" onclick="openModal(' . htmlspecialchars(json_encode($row)) . ')">';
...
function openModal(data) {
  document.getElementById("modalContent").innerHTML = `
    <img src="../Backend/uploads/${data.AdminProfile}" alt="Avatar" class="w-20 h-20 rounded-full mx-auto mb-1 border-2 border-inherit">
    <h2 class="text-lg font-bold">${data.Fname} ${data.MName} ${data.Lname}</h2>
    <p>Position: ${data.BrgyPosition}</p>
    <p>Gender: ${data.AdGender}</p>
    <p>Service Start: ${data.RenderedServiceStart}</p>
    <p>Service End: ${data.RenderedServiceend}</p>
  `;
