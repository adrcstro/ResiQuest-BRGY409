...
<div  id="ResidentsMessage"  class="absolute bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
  <div class="pt-3 px-4">
    ...
    <img src="/Images/brgylogo.png" alt="Brenda S. Puertollanoo" class="w-32 h-32 rounded-full mx-auto">
    <h2 class="mt-4 text-lg font-bold">BARANGAY-409</h2>
    <p class="text-gray-500">Barangay-409 Sta, Teresita Sampaloc, Manila </p>
  </div>
  <form id="residentmessage" class="mt-12" onsubmit="displayValues(event)">
    <label for="chat" class="sr-only">Your message</label>
    <div class="flex items-center py-2 px-3 bg-gray-50 rounded-lg dark:bg-gray-700">
    
    <div class="relative ">
  <input value="<?php echo $row["Profile"] ?>" type="file" id="PRofileMEssagesender" name="PRofileMEssagesender"  accept=".jpg, .png .jpeg .pdf, .doc, .docx">
</div>

<div class="relative">
  <input  type="file" id="PRofileMEssagesender" name="PRofileMEssagesender" class="hidden" accept=".jpg, .png .jpeg .pdf, .doc, .docx" onchange="displaySelectedImage(event)">


  <div class="relative">
    <?php if(isset($row["Profile"]) && !empty($row["Profile"])): ?>
        <img id="PRofileMEssagesender" src="../Backend/uploads/<?php echo $row["Profile"]; ?>" class="w-32 h-32 bg-gray-300 rounded-full  mb-4 border-4" alt="Profile Picture">
    <?php else: ?>
        <img id="PRofileMEssagesender"  src="/Images/blank.png" class="w-32 h-32 bg-gray-300 rounded-full mb-4" alt="Default Profile Picture">
    <?php endif; ?>
</div>


<label for="PRofileMEssagesender" class="absolute bottom-0 right-0 p-2 bg-gray-100 text-black rounded-full hover:bg-gray-400 focus:outline-none focus:bg-gray-400 cursor-pointer border-4 " title="Edit Profile">
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6">
    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
  </svg>
</label>
    ...
    <div class="mt-2.5 hidden">
    <input value="<?php echo $row["ID"]?>" required="" type="text" name="ResidentIDMessage" id="ResidentIDMessage" autocomplete="given-name" placeholder="ResidentIDMessage" 
    class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
    </div>
    <div class="mt-2.5 hidden">
    <input type="text" name="SenderFullname" value="<?php echo $row['fname'] . ' ' . $row['Mname'] . ' ' . $row['lname']; ?>" class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
    </div>
        <div class="mt-2.5 ml-2 hidden">
            <input type="text" name="date" id="date" placeholder="Current Date" readonly class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
        </div>
        <div class="mt-2.5 ml-2 hidden">
            <input type="text" name="time" id="time" placeholder="Current Time" readonly class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
        </div>
        <input type="file" id="image-upload-Message" name="image-upload-Message" class="hidden" accept="image/*">
        <label for="image-upload-Message" class="inline-flex justify-center p-2 text-gray-500 rounded-lg cursor-pointer hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600">
        ...
        <textarea id="chat" name="chat" rows="1" class="block mx-4 p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your message..."></textarea>
        <button id="Sendmessage" type="submit" class="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer hover:bg-blue-100 dark:text-blue-500 dark:hover:bg-gray-600">
...
<script>
document.getElementById('image-upload-Message').addEventListener('change', function(event) {
    const file = event.target.files[0];
    const reader = new FileReader();
    reader.onload = function(e) {
        ...
        // Clear any existing image and add the new one
        const existingImage = document.getElementById('chat-image');
        if (existingImage) {
            existingImage.remove();
        }
        
        img.id = 'chat-image';
        chat.parentNode.insertBefore(img, chat);
    ...
    if (file) {
        ...
        // Reduce the height of the textarea
        document.getElementById('chat').rows = 2;
    }
});
...
$(function() {
    $('#Sendmessage').click(function(e) {
        e.preventDefault();
        var valid = $('#residentmessage')[0].checkValidity();
        if (valid) {
            var formData = new FormData($('#residentmessage')[0]);
            $.ajax({
                type: 'POST',
