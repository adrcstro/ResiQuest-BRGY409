    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
          if ($row["Status"] !== "Completed") {
            ...
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900 ">' . $row["Status"] . '</td>';
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["PickUpDate"] . '</td>';
            echo '<td class="p-4 whitespace-nowrap text-sm font-semibold text-gray-900 relative">
            <button id="actions-btn-' . $row["RequestID"] . '" class="open-modal-btn">Actions</button>
            <div id="actions-menu-' . $row["RequestID"] . '" class="absolute hidden bg-white shadow-md rounded-md py-2 w-32 right-0 mt-2">
              <a href="#" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">View Details ' . $row["RequestID"] . '</a>
              <a href="#" class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Edit</a>
            </div>
          </td>';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".open-modal-btn").forEach(function (button) {
    button.addEventListener("click", function () {
      const requestId = button.getAttribute("data-request-id");
      const menu = document.getElementById("actions-menu-" + requestId);
