             ...
             <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                <div class="flex-1 px-3 bg-white divide-y space-y-1">
                   <ul class="space-y-2 pb-2">
                      ...
                      <li>
                         <a href="BlotterReport.php"  class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group" >
                      ...
                      <li>
                         <a href="PublishNews.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group" >
                           <svg  class="w-6 h-6 text-gray-500 flex-shrink-0 group-hover:text-gray-900 transition duration-75" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                      ...
                      <li id="superadminonlyaccess">
                         <a href="BrgyOfficials.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group" >
                           <svg class="w-6 h-6 text-gray-500 flex-shrink-0 group-hover:text-gray-900 transition duration-75" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                      ...
                      <li>
                         <a href="Message.php" class="text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group">
                           <svg class="w-6 h-6 text-gray-500 flex-shrink-0 group-hover:text-gray-900 transition duration-75" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
...
<!-- Message -------------------------------------------------------------------------------------------------------------->
<div  id="showBrgyOfficials"   class="absolute  bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
   <div class="pt-3 px-4">
      <div class="bg-white  shadow rounded-lg p-4 sm:p-6 xl:p-8  xl:gap-4 my-4">
        <div class="mb-4 flex flex-col lg:flex-row items-center justify-between">
               <div class="mb-4 lg:mb-0">
                   <h3 class="text-xl font-bold text-gray-900 mb-2">Message & Resident Concern</h3>
...
// SQL query to fetch data from the table and order by Time in ascending order
$sql = "SELECT * FROM messages ORDER BY Time ASC";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    // Initialize an array to hold messages grouped by SenderID
    $messages = [];
    
    // Fetch data and group by SenderID
    while($row = $result->fetch_assoc()) {
        $messages[$row["SenderID"]][] = $row;
    }

    // Set up the grid container
    echo "<div class='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>"; // Responsive grid
    // Iterate over grouped messages and display in one card
    foreach($messages as $senderID => $senderMessages) {
        $messagesJson = json_encode($senderMessages);  // Convert messages to JSON for JavaScript
        $senderProfile = htmlspecialchars($senderMessages[0]["SenderProfile"] ?? '');
        $senderProfile = empty($senderProfile) ? '/Images/blank.png' : '../Backend/uploads/' . $senderProfile; // Check if profile is empty
        $senderName = htmlspecialchars($senderMessages[0]["SenderName"]);
        $encodedSenderProfile = json_encode($senderProfile);
        $encodedSenderName = json_encode($senderName);
        $isViewed = true;
        foreach ($senderMessages as $message) {
            if (!$message['viewed']) {
                // If any message is not viewed, set $isViewed to false and break the loop
                $isViewed = false;
                break;
            }
        } // Check if the message has been viewed
        $isViewedClass = $isViewed ? 'hidden' : '';
        echo "<div class='relative border border-gray-200 rounded-lg shadow-lg'>";
        echo "<span id='newMessageBadge-{$senderID}' class='absolute top-0 left-0 p-1 bg-red-500 text-white text-xs font-bold rounded $isViewedClass'>New Message</span>";
        ...
        $senderProfile = "path/to/profile/image.jpg"; // replace with the actual path to the sender's profile image
        echo "<img class='object-cover w-12 h-12 rounded-full border border-gray-500' src='" . $senderProfile . "' alt='Sender Profile' />";
        echo "<div class='ml-3 overflow-hidden'>";
        echo "<p class='font-medium text-gray-900'>" . $senderName . "</p>";
        echo "<p class='max-w-xs text-sm text-gray-500 truncate'>Resident ID: " . htmlspecialchars($senderID) . "</p>";
        echo "<button class='mt-2 p-2 bg-blue-500 text-white rounded' onclick='openModal(" . $messagesJson . ", " . $encodedSenderProfile . ", " . $encodedSenderName . ", " . htmlspecialchars($senderID) . ")'>View Message</button>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
    }
    ...
    echo "<p class='text-lg font-semibold text-center '>No New  Message Found</p>";
    echo '</div>';
}
...
<!-- DAISY UI Modal Structure -->
<dialog id="messageModal" class="modal">
    <div class="modal-box w-100 max-w-4xl rounded-lg">
        <form method="dialog">
            <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
        </form>
        <div id="modalSender" class="text-center mb-4"></div> <!-- Added container for sender info -->
        <div id="modalContent" class="flex flex-col max-h-[80vh] overflow-y-auto"></div> <!-- Added flex-col and max height class -->
    </div>
</dialog>

<script>
function openModal(messages, senderProfile, senderName, senderID) {
    const modal = document.getElementById('messageModal');
    const modalSender = document.getElementById('modalSender');
    const modalContent = document.getElementById('modalContent');

    modalSender.innerHTML = ''; // Clear previous sender info
    modalContent.innerHTML = ''; // Clear previous content

    // Add sender information at the top center
    const senderElem = document.createElement('div');
    senderElem.classList.add('mb-4', 'text-center');
    
    const profileImg = document.createElement('img');
    profileImg.src = senderProfile;
    profileImg.alt = 'Sender Profile';
    profileImg.classList.add('mx-auto', 'w-12', 'h-12', 'rounded-full');
    senderElem.appendChild(profileImg);
    
    const senderNameElem = document.createElement('p');
    senderNameElem.textContent = senderName;
    senderNameElem.classList.add('text-lg', 'font-medium', 'text-gray-900');
    senderElem.appendChild(senderNameElem);

    const senderIDElem = document.createElement('p');
    senderIDElem.textContent = 'Resident ID: ' + senderID;
    senderIDElem.classList.add('text-sm', 'text-gray-500');
    senderElem.appendChild(senderIDElem);

    modalSender.appendChild(senderElem);
    ...
    messages.forEach(message => {
        ...
        messageElem.classList.add('mb-4');

        const msgText = document.createElement('p');
        msgText.textContent = message.msg;
        msgText.classList.add('text-sm', 'text-gray-500', 'break-words'); // Added break-words class
        messageElem.appendChild(msgText);

        if (message.MessageImage) {
            const msgImage = document.createElement('img');
            msgImage.src = '../Backend/uploads/' + message.MessageImage;
            msgImage.alt = 'Message Image';
            msgImage.classList.add('mt-2', 'w-20', 'h-20', 'object-cover', 'rounded'); // Adjusted classes for image
            messageElem.appendChild(msgImage);
        }

        const msgDate = document.createElement('p');
        msgDate.textContent = message.Date + ' ' + message.Time;
        msgDate.classList.add('text-xs', 'text-gray-400');
        messageElem.appendChild(msgDate);

        modalContent.appendChild(messageElem);
    });

    modal.showModal();
    modalContent.scrollTop = modalContent.scrollHeight; // Scroll to the bottom of the modal content

    // Hide the "New Message" badge for this sender
    const newMessageBadge = document.getElementById(`newMessageBadge-${senderID}`);
    if (newMessageBadge) {
        newMessageBadge.style.display = 'none';
    }

    // Send an AJAX request to update the viewed status of the messages
    fetch('update_viewed_status.php', {
        method: 'POST',
        headers: {
