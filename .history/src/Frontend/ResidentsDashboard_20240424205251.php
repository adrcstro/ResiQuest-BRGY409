...
<div  id="ResidentsDocumentsRequested"  style="display: none;" class="absolute bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
    ...
    <div class="bg-white py-4 px-5 text-center rounded-md shadow-2xl transform -translate-y-20 sm:-translate-y-24 max-w-sm mx-auto mt-2 sm:m-2">
        ...
        <div>
            <img src="/Images/residency.png" alt="Image Icon" style="width: 40px; height: 40px;">
        </div>
...
<div class="flex items-center">
  ...
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    <div class="mt-2.5">
    <select id="CivilStatisbrgyID" name="CivilStatisbrgyID" autocomplete="family-name"
    class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
    <?php if (!empty($row["CivilStatus"])): ?>
        <option value="<?php echo $row["CivilStatus"]; ?>" selected><?php echo $row["CivilStatus"]; ?></option>
    <?php else: ?>
  ...
  <div class="mt-4 grid grid-cols-1 gap-y-6 gap-x-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 2xl:grid-cols-3">
    ...
    <div>
        ...
        <div class="mt-2">
            <input required="" type="text" name="ContactpersonnumberbrgyID" id="ContactpersonnumberbrgyID" autocomplete="family-name"
...
<script>
  $(document).ready(function(){
    $("#submitBarangayID").click(function(){
        ...
        // Check if any field is empty
     // Check if any field is empty
if (!firstnamebrgyID || !MnamebrgyID || !lastnamebrgyID || !DateofbirthbrgyID || !PlaceofBirthbrgyID || !CivilStatisbrgyID || !SSSGSISbrgyID || !DaterequestedbrgyID || !AddressbrgyID || !ContactpersonNamebrgyID || !ContactpersonAddressbrgyID || !ContactpersonnumberbrgyID || !ServicetypebrgyID) {
    Swal.fire({
        title: 'Error!',
