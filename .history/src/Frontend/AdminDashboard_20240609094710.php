...
<script>
    function loadTable(page, status) {
        $.ajax({
            url: '../Backend/AdminTables/activitylog.php',
            type: 'GET',
    ...
    $(document).ready(function() {
        // Assuming status is passed via query parameter in the URL
        const urlParams = new URLSearchParams(window.location.search);
        const status = urlParams.get('status');

        // Load the first page with the given status
        loadTable(1, status);
