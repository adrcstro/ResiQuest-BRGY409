        ...
        <span class="text-base font-normal text-gray-500">This is a list Released Documents</span>
             ...
             <div class="flex-shrink-0">
                 ...
                 <div class="lg:flex lg:items-center lg:pl-32">
                     <button class="mt-3 w-full relative text-white border-2 border-gray-200  bg-green-400 dark:bg-gray-800 dark:border-gray-700 hover:bg-green-500  font-semibold py-2 px-4">
...
<nav class="flex space-x-8 justify-center px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
    <ol class="inline-flex items-end space-x-4 md:space-x-6 rtl:space-x-reverse">
        <li class="inline-flex items-center">
            <a href="#" onclick="applyFilter('empty'); return false;" class="inline-flex items-center text-sm font-medium text-gray-700 focus:outline-none focus:text-blue-600 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                Set Schedule
            </a>
        </li>
        <li>
            <div class="flex items-center">
                <a href="#" onclick="applyFilter('future'); return false;" class="text-sm font-medium text-gray-700 focus:outline-none focus:text-blue-600 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">View Future's Schedule</a>
            </div>
        </li>
        <li>
            <div class="flex items-center">
                <a href="#" onclick="applyFilter('today'); return false;" class="text-sm font-medium text-gray-700 focus:outline-none focus:text-blue-600 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">View Today's Schedule</a>
            </div>
        </li>
        <li>
            <div class="flex items-center">
                <a href="#" onclick="applyFilter('overdue'); return false;" class="ms-1 text-sm font-medium text-gray-700 focus:outline-none focus:text-blue-600 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">View Over Due</a>
            </div>
        </li>
    </ol>
</nav>
...
<div id="table-container">
    <!-- The PHP generated table will be inserted here -->
</div>
...
// Determine which filter to apply based on the query parameter
...
switch ($filter) {
    case 'empty':
        ...
        break;
    case 'future':
        $query = "SELECT * FROM requestdocument WHERE Status='Completed' AND PickUpDate > '$today'";
        break;
    case 'today':
        $query = "SELECT * FROM requestdocument WHERE Status='Completed' AND PickUpDate = '$today'";
        break;
...
if (mysqli_num_rows($result) > 0) {
    $table_html = '<div class="overflow-x-auto h-screen"><table class="table-auto min-w-full divide-y divide-gray-200 mt-3">
    <thead class="bg-gray-50">
        <tr>
            <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Request ID</th>
            <th scope="col" class="p-4 border-2 border-inherit text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Residents Name</th>
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $table_html .= '<tr>';
        $table_html .= '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["RequestID"]) . '</td>';
        $table_html .= '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["FirstName"] . ' ' . $row["Middlename"] . ' ' . $row["Lastname"]) . '</td>';
        $table_html .= '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["ServiceType"]) . '</td>';
        $table_html .= '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["PickUpDate"]) . '</td>';
        $table_html .= '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . htmlspecialchars($row["Pickuptime"]) . '</td>';
        $table_html .= '<td class="p-1 border flex items-center justify-center border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">
            <div class="dropdown dropdown-end">
                <div tabindex="0" role="button" class="flex items-center justify-between text-white font-semibold btn w-40 bg-blue-400 hover:bg-blue-500">
                    Action Details
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
                    ...
                    <li class="ReSchedule-resident-request" data-request-id="' . htmlspecialchars($row["RequestID"]) . '" data-request-servietype="' . htmlspecialchars($row["ServiceType"]) . '" data-request-Fname="' . htmlspecialchars($row["FirstName"]) . '" data-request-Mname="' . htmlspecialchars($row["Middlename"]) . '" data-request-Lname="' . htmlspecialchars($row["Lastname"]) . '" data-reschedule-date="' . htmlspecialchars($row["PickUpDate"]) . '" data-reschedule-time="' . htmlspecialchars($row["Pickuptime"]) . '">
                        <a>Reschedule</a>
                    </li>
                    <li class="mark-as-released" data-request-id="' . htmlspecialchars($row["RequestID"]) . '" data-request-servietype="' . htmlspecialchars($row["ServiceType"]) . '" data-request-Fname="' . htmlspecialchars($row["FirstName"]) . '" data-request-Mname="' . htmlspecialchars($row["Middlename"]) . '" data-request-Lname="' . htmlspecialchars($row["Lastname"]) . '">
                        <a>Mark Released</a>
                    </li>
                </ul>
            </div>
...
dialog.close();
    ...
    $table_html = '<div class="mx-auto text-center mt-8" style="width: 200px;">';
    $table_html .= '<img src="/Images/norequest.svg" alt="Illustration" class="w-30 h-auto">';
    $table_html .= "<p class='text-lg font-semibold text-center'>No Request Found</p>";
    $table_html .= '</div>';
}
...
if ($is_ajax) {
    ...
    // Otherwise, return the full page with the table
    echo '<!DOCTYPE html>
    <html>
    <head>
   ...
   <div class="pt-3 px-4">
      ...
      <div class="mb-4 lg:mb-0">
             ...
             <form action="#" method="GET" class="lg:flex lg:items-center lg:pl-32">
                  ...
                  <div class="lg:flex lg:items-center lg:pl-32">
                  <button onclick="addadminnistrator.showModal()" class="mt-2 w-full relative text-white border-2 border-gray-200rounded-lg bg-green-400 dark:bg-gray-800 dark:border-gray-700 hover:bg-green-500  font-semibold py-2 px-4">
 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="w-5 h-5 mr-1 inline-block align-middle text-white">
  <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
</svg>
Add Officials
</button>
