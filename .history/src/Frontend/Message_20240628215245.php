...
if ($result->num_rows > 0) {
    ...
    foreach($messages as $senderID => $senderMessages) {
        ...
        $isViewed = $senderMessages[0]['viewed'] ? 'hidden' : '';  // Check if the message has been viewed
        echo "<div class='relative border border-gray-200 rounded-lg shadow-lg'>";
        echo "<span id='newMessageBadge-{$senderID}' class='absolute top-0 left-0 p-1 bg-red-500 text-white text-xs font-bold rounded $isViewed'>New Message</span>";
