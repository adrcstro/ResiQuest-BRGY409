...
$delete_query = "DELETE FROM archive_my_table WHERE created_at < NOW() - INTERVAL 3 DAY";
...
$query = "SELECT ID,fname,Mname,lname,ContactNumber,Gender,email,password,Nickname,PlaceofBirth,Dateofbirth,Age,CivilStatus,Purok,VoterStatus,Occupation,Address
Zipcode,Citezenship,Profile,GovernmentIDFront,GovernmentIDBack,Status FROM archive_my_table LIMIT $limit OFFSET $offset";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
       ...
       <button id="revertbtn" onclick="revertRecord(' . $row["ID"] . ')" class="px-3 py-2 bg-blue-500 text-white rounded flex items-center justify-center space-x-2 w-full">
  ...
  <span>Revert Action</span>
    ...
    echo '</ul></nav>';
    echo'<div class="bg-blue-100 p-3 rounded-md mt-4 ">
    <div class="flex items-center justify-center">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
                  ...
                  <p class="text-center font-semibold"  >This table will contain all of the deleted resident data.
                   Before it is permanently archived, you will have (3) three days 
                   to examine and confirm if the Information will be Permanently Deleted. You have 
                   this time to make any necessary updates or corrections to guarantee 
                   that all records are true and comprehensive. Following the three-day
                review period, the data will be Permanently Deleted in the System.</p>
    </div>';
} else {
    ...
    echo "<p class='text-lg font-semibold text-center'>No Deleted Residents Found</p>";
    echo '</div>';
}
?>
