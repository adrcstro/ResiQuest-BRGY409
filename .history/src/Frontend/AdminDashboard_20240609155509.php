...
<div id="residents-table"></div>
    ...
    $(document).ready(function() {
        function loadResidents(page = 1) {
            $.ajax({
                url: 'fetch_residents.php',
                type: 'GET',
                data: { page: page },
                dataType: 'json',
                success: function(response) {
                    if (response.error) {
                        $('#residents-table').html('<div class="error">' + response.error + '</div>');
                        return;
                    }
                    var rows = '';
                    $.each(response.data, function(index, row) {
                        ...
                        rows += '<tr>';
                        rows += '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' + row.ID + '</td>';
                        rows += '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' + row.fname + ' ' + row.Mname + ' ' + row.lname + '</td>';
                        rows += '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' + row.Age + '</td>';
                        rows += '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' + row.Gender + '</td>';
                        rows += '<td class="p-3 text-center border-2 border-inherit rounded-md text-sm font-semibold text-gray-900 ' + statusBgColor + '">' + row.Status + '</td>';
                        rows += '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">';
                        rows += '<div class="dropdown dropdown-end">';
                        rows += '<div tabindex="0" role="button" class="flex items-center justify-between text-white font-semibold btn w-40 bg-blue-400 hover:bg-blue-500">';
                        rows += 'Action Details';
                        rows += '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" />';
                        rows += '</svg>';
                        rows += '</div>';
                        rows += '<ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52 ">';
                        rows += '<li class="open-resident-details-modal-btn" data-Residents-id="' + row.ID + '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />';
                        rows += '</svg> View Details</a></li>';
                        rows += '<li class="edit-residents-btn" data-Residents-id="' + row.ID + '" data-Residents-profile="' + row.Profile + '" data-Residents-fname="' + row.fname + '" data-Residents-mname="' + row.Mname + '" data-Residents-Lname="' + row.lname + '" data-Residents-nname="' + row.Nickname + '" data-Residents-placeofbirth="' + row.PlaceofBirth + '" data-Residents-dateofbirth="' + row.Dateofbirth + '" data-Residents-age="' + row.Age + '" data-Residents-civilstat="' + row.CivilStatus + '" data-Residents-gender="' + row.Gender + '" data-Residents-purok="' + row.Purok + '" data-Residents-voterstat="' + row.VoterStatus + '" data-Residents-email="' + row.email + '" data-Residents-occupation="' + row.Occupation + '" data-Residents-contactnum="' + row.ContactNumber + '" data-Residents-address="' + row.Address + '" data-Residents-zipcode="' + row.Zipcode + '" data-Residents-citizenship="' + row.Citezenship + '" data-adminid-logs="' + row.AdminID + '" data-adminusername-logs="' + row.lname + ',' + row.fname + ' ' + row.MName + ' " data-adminposition-logs="' + row.BrgyPosition + '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />';
                        rows += '</svg> Edit Residents</a></li>';
                        rows += '<li class="delete-residents-btn" data-Residents-id="' + row.ID + '" data-adminid-logs="' + row.AdminID + '" data-adminusername-logs="' + row.lname + ',' + row.fname + ' ' + row.MName + ' " data-adminposition-logs="' + row.BrgyPosition + '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="M6 18 6 19.5 18 19.5 18 18M10.5 6 10.5 3.75c0-.414.336-.75.75-.75h1.5c.414 0 .75.336.75.75L13.5 6M10.5 6l-3 12m3-12h3m-3 0 3 12m0 0 3-12m-3 12H7.5" />';
                        rows += '</svg> Delete Residents</a></li>';
                        rows += '<li class="verify-residents-btn" data-Residents-id="' + row.ID + '"><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">';
                        rows += '<path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />';
                        rows += '</svg> Verify Residents</a></li>';
                        rows += '</ul>';
                        rows += '</div>';
                        rows += '</td>';
                        rows += '</tr>';
                    });
                    $('#residents-table').html('<table class="min-w-full divide-y divide-gray-200"><thead><tr><th>ID</th><th>Name</th><th>Age</th><th>Gender</th><th>Status</th><th>Actions</th></tr></thead><tbody>' + rows + '</tbody></table>');

                    var pagination = '';
                    for (var i = 1; i <= response.total_pages; i++) {
                        pagination += '<button class="pagination-button" data-page="' + i + '">' + i + '</button> ';
                    }
