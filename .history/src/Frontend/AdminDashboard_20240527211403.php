...
if (!empty($_SESSION["AdminID"])) {
    ...
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $totalCountonprocess = $row["pending_countblotter"];
    } else {
        echo "Error retrieving data from the database.";
    }
    $conn->close();
...
if (!empty($_SESSION["AdminID"])) {
    ...
    $sql = "SELECT COUNT(*) AS pending_countblotter FROM complaintbl WHERE ComplainStatus = 'On-Schedule'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $totalCouncompleted = $row["pending_countblotter"];
    } else {
        echo "Error retrieving data from the database.";
    }
    $conn->close();
