...
<div  id="ResidentsMessage"  style="display: none;" class="absolute bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
  ...
  <div class="modal-box w-full max-w-6xl rounded-none">
    ...
    // Prepare and execute a query to fetch the BLOB data for the two files
    $stmt = $conn->prepare("SELECT GovernmentID FROM users");
    $stmt->execute();
    $result = $stmt->get_result();

    // Fetch the BLOB data for each file
    while ($row = $result->fetch_assoc()) {
        $blobData = $row['GovernmentID'];

        // Process the BLOB data (e.g., convert it to an image)
        // For example, if you're working with image data:
        $imageData = base64_encode($blobData);

        // Display the image in the dialog
        echo '<img src="../Backend/uploads/jpeg;base64,' . $imageData . '" class="w-32 h-32 bg-gray-300 rounded-none mb-4" alt="ID">';
    }
