...
$sql = "SELECT *, (SELECT COUNT(*) FROM messages AS m2 WHERE m2.SenderID = messages.SenderID AND m2.is_new = 1) AS new_message_count FROM messages ORDER BY Time ASC";
...
if ($result->num_rows > 0) {
    ...
    foreach($messages as $senderID => $senderMessages) {
        ...
        // Count new messages
        $newMessageCount = array_reduce($senderMessages, function($count, $message) {
            return $count + ($message['is_new'] == 1 ? 1 : 0);
        }, 0);

        echo "<div class='relative border border-gray-200 rounded-lg shadow-lg'>";
        
        // Add the notification badge if there are new messages
        if ($newMessageCount > 0) {
            echo "<div class='absolute top-0 left-0 m-2 bg-red-600 text-white rounded-full text-xs w-6 h-6 flex items-center justify-center'>$newMessageCount</div>";
        }
