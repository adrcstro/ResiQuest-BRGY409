...
if (!empty($_SESSION["AdminID"])) {
    $ID = $_SESSION["AdminID"];
    $stmt = $conn->prepare("SELECT * FROM admintbl WHERE AdminID = ?");
    if ($stmt) {
        ...
        if ($row) {
            $_SESSION['BrgyPosition'] = $row['BrgyPosition'];
        } else {
...
<div>
    <nav class="bg-white border-b border-gray-200 fixed z-30 w-full">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-5 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
         ...
         <?php if (isset($_SESSION['BrgyPosition']) && $_SESSION['BrgyPosition'] == 'Barangay Chairman'): ?>
