...
if (isset($_POST["submit"])) {
    ...
    if (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($newPassword) < 8) {
        ...
        $stmt = $conn->prepare("SELECT ContactNumber FROM users WHERE email = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($storedContactNumber);
        $stmt->fetch();
        ...
        if ($stmt->num_rows == 0) {
            ...
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire(
                    'Error',
                    'Email you typed is not registered in our system',
                    'error'
                );
            });
         </script>";
            ...
            if ($newPassword !== $confirmPassword) {
                ...
                if ($contactNumber !== $storedContactNumber) {
                    echo "<script>
                        document.addEventListener('DOMContentLoaded', function() {
                            Swal.fire(
                                'Error',
                                'Contact number does not match to the <br> Contact Number you registered',
                                'error'
                            );
                        });
                     </script>";
...
<!-- login -------------------------------------------------------------------------------------------------------------->
        ...
        <div class="lg:w-1/2 xl:w-5/12 p-6 sm:p-12">
            ...
            <div class="flex items-center justify-center gap-4 mt-4">
               ...
               <div class="text-center mt-3">
                <span class="mb-3 text-2xl font-bold text-blue-800">Forgot Password<br></span>
                    ...
                    <div class="relative">
                        ...
                        <div class="mt-3 mb-2">
                                ...
                                <div class="relative">
                                    <input
                                        class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white pl-12"
                                        type="number" name="ContactNumber" placeholder="+63" />
                                          <img
                                          src="/Images/flg.svg"
    ...
    <h2 class="text-lg font-semibold text-gray-600">New Password Setup</h2>
    <p class="text-sm text-gray-600">Setup New Password</p>
</div>
    ...
    <p class="text-sm text-gray-600">Setup Strong User Password</p>
</div>
        
                    <!-- Illustration 3 -->
                    <div class="bg-white p-4 rounded-lg shadow-2xl flex flex-col items-center">
    <img src="/Images/no.svg" alt="Illustration" class="w-32 h-auto">
    <h2 class="text-lg font-semibold text-gray-600">Choosing a Strong Password</h2>
    <p class="text-sm text-gray-600">Avoid using Common Password</p>
</div>

        
                    <!-- Illustration 4 -->
                    <div class="bg-white p-4 rounded-lg shadow-2xl flex flex-col items-center">
    <img src="/Images/update.svg" alt="Illustration" class="w-32 h-auto">
    <h2 class="text-lg font-semibold text-gray-600">Monitor Resident Status</h2>
    <p class="text-sm text-gray-600">Monitor Account Status</p>
</div>
