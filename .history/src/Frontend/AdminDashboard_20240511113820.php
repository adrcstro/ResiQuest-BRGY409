...
Complete Request
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
    ...
    <select id="scheduleFilter" class="rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
        ...
        <?php
        // PHP code to generate options based on available schedules
        $conn = mysqli_connect($servername, $username, $password, $dbname);
        if (!$conn) {
            echo "Database connection error" . mysqli_connect_error();
        }
        $query = "SELECT DISTINCT PickUpDate FROM requestdocument WHERE Status='Completed'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                echo '<option value="' . $row["PickUpDate"] . '">View Schedule for ' . $row["PickUpDate"] . '</option>';
            }
        }
        ?>
    </select>
...
<div id="Completedrequesttable" class="hidden"> 
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Fetch and display each row
        while ($row = mysqli_fetch_assoc($result)) {
            echo '<tr data-scheduled-for="' . $row["PickUpDate"] . '">';
            echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["RequestID"] . '</td>';
            echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["FirstName"] . ' ' . $row["Middlename"] . ' ' . $row["Lastname"] . '</td>';
            echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["ServiceType"] . '</td>';
            echo '<td class="p-3 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">' . $row["PickUpDate"] . '</td>';
            ...
            echo '<td class="p-1 border border-inherit text-center whitespace-nowrap text-sm font-semibold text-gray-900">
               ...
               <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow-2xl bg-base-100 rounded-box w-52">
                  ...
                  Schedule Released</a></li>
                   <li class="ReSchedule-resident-pickup" data-request-id="' . $row["RequestID"] . '"  data-request-servietype="' . $row["ServiceType"] . '"  data-request-Fname="' . $row["FirstName"] . '"  data-request-Mname="' . $row["Middlename"] . '" data-request-Lname="' . $row["Lastname"] . '" ><a><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<script>
    ...
    scheduleFilter.addEventListener('change', function() {
        ...
        rows.forEach(row => {
            if (selectedValue === 'all' || (selectedValue === 'today' && row.dataset.scheduledFor === today)) {
                row.style.display = '';
            } else {
    ...
    // Initialize the table to show today's schedule by default
