...
<div  id="ResidentsNotofication"   class="absolute bg-gray-50 inset-0 overflow-y-auto lg:ml-64 mt-16">
   <div class="pt-3 px-4">
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<button class='btn btn-sm btn-primary mt-2' onclick=\"openModal('$newsId')\">View</button>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
        ...
        // Modal Dialog
          // Modal Dialog
    echo "<div id='$newsId' class='modal'>";
    echo "<div class='modal-box max-w-5xl relative'>"; // Added relative positioning for modal-box
    // Close button
    echo "<button class='absolute top-2 right-2 p-1 bg-gray-100 border border-gray-300 rounded-full' onclick=\"closeModal('$newsId')\">";
    echo "<svg xmlns='http://www.w3.org/2000/svg' class='w-5 h-5' viewBox='0 0 20 20' fill='currentColor'>";
    echo "<path fill-rule='evenodd' d='M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z' clip-rule='evenodd' />";
    echo "</svg>";
    echo "</button>";
    echo "<div class='mt-4 px-4 py-3 sm:px-6 md:px-8 lg:px-10 flex flex-col md:flex-row gap-4'>";
    echo "<div class='flex-1 flex flex-col justify-between'>";
    echo "<div class='flex flex-col items-center justify-center w-full h-full'>";
    echo "<div class='text-lg font-bold text-center'>";
    echo "<h2>News Thumbnails</h2>";
    echo "<p class='text-xs text-gray-500 dark:text-gray-400 mb-2'>SVG, PNG, JPG, or GIF (MAX. 800x400px)</p>";
    echo "</div>";
    echo "<div class='flex flex-col items-center justify-center w-full h-full p-2 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500'>";
    echo "<img src='../Backend/uploads/" . htmlspecialchars($row["NewsImages"]) . "' alt='News Image' class='w-full h-full object-cover border-2 border-gray-400 rounded-md p-2'>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    echo "<div class='flex-1 flex flex-col justify-between'>";
    echo "<div>";
    echo "<span class='block text-sm font-semibold leading-6 text-gray-900'>News Title</span>";
    echo "<input value='" . htmlspecialchars($row["NewsTittle"]) . "' type='text' name='NewsTitle' placeholder='Enter News Title' class='input input-bordered focus:outline-none focus:ring-1 focus:border-blue-500 input-white w-full bg-white text-slate-900' readonly />";
    echo "</div>";
    echo "<div class='mt-3 flex flex-col sm:flex-row gap-x-6'>";
    echo "<div class='sm:flex-1'>";
    echo "<label for='NewsDate' class='block text-sm font-semibold leading-6 text-gray-900'>News Date</label>";
    echo "<div>";
    echo "<input value='" . htmlspecialchars($row["NewsDate"]) . "' required type='date' name='NewsDate' id='NewsDate' autocomplete='Address' placeholder='Enter Address' disabled class='block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 flex-1 readonly'>";
    echo "</div>";
    echo "</div>";
    echo "<div class='sm:flex-1'>";
    echo "<label for='NewsTime' class='block text-sm font-semibold leading-6 text-gray-900'>News Time</label>";
    echo "<div>";
    echo "<input value='" . htmlspecialchars($row["Newstime"]) . "' required type='time' name='NewsTime' id='NewsTime' autocomplete='Zip-Code' placeholder='Enter City Zip Code Ex. 1008' disabled class='block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 flex-1 readonly'>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    echo "<div class='mt-3'>";
    echo "<span class='block text-sm font-semibold leading-6 text-gray-900'>News Content</span>";
    echo "<textarea name='NewsContent' rows='6' class='rounded-lg border-gray-200 w-full h-64 pt-2 text-gray-900 border-1 bg-white input input-bordered focus:outline-none focus:ring-1 focus:border-blue-500 resize-none' placeholder='Enter News Content.....' readonly>" . htmlspecialchars($row["NewsContent"]) . "</textarea>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
...
function openModal(newsId) {
    document.getElementById(newsId).classList.add('modal-open');
}

function closeModal(newsId) {
    document.getElementById(newsId).classList.remove('modal-open');
}
</script>
